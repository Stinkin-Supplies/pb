 ...
 * Stores JSONB batches into raw_vendor_pu on the catalog DB (Hetzner Postgres).
 *
 * Usage:
  ...
  for await (const line of rl) {
    ...
    if (batch.length >= BATCH_SIZE) {
      ...
      try {
        ...
        process.stdout.write(`\r✓ Batch ${batchNum} - ${importedRows.toLocaleString()} rows imported`);
      } catch (err) {
        failedBatches++;
