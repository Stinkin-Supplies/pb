...
const SCHEMA = {
  ...
  fields: [
    ...
    { name: 'our_price',    type: 'float',   facet: true  },
...
async function run() {
  ...
  while (true) {
    const { rows } = await db.query(`
      SELECT
        ...
        (SELECT ci.url FROM public.catalog_images ci
         ...
         LIMIT 1)                                      AS image,
        ARRAY(
          SELECT vo.vendor_code FROM public.vendor_offers vo
    ...
    const docs = rows.map(r => ({
      ...
      our_price:    Number(r.our_price || r.price || 0),
      ...(r.map_price != null ? { map_price: Number(r.map_price) } : {}),
      ...(r.msrp      != null ? { msrp:      Number(r.msrp)      } : {}),
      is_active:    Boolean(r.is_active),
      stock_quantity: Number(r.stock_quantity || 0),
      in_stock:     Boolean(r.in_stock ?? Number(r.stock_quantity || 0) > 0),
    ...
    try {
      ...
      for (const result of results) {
        if (result.success) indexed++;
          ...
          failed++;
          if (failed <= 3) console.error('\n  ❌ ', result.error, result.document?.id);
        }
      }
    } catch (err) {
  ...
  console.log(`\n   Collection doc count: ${Number(info.num_documents).toLocaleString()}`);
