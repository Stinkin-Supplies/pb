#!/usr/bin/env node
// audit_accessories_misc_wave3.mjs
//
// Third discovery pass on whatever remains NULL in Accessories & Misc
// after wave-1 (fix_accessories_misc_taxonomy.mjs) and wave-2
// (fix_accessories_misc_wave2.mjs). Expect ~1,088 rows plus the 31
// explicitly-flagged ambiguous rows from wave-2 (those are excluded
// from wave-2's classifier but still sit in the table with NULL
// subcategory, so they'll show up here too unless filtered out).
//
// Read-only. No writes.
//
// Usage:
//   node audit_accessories_misc_wave3.mjs           (summary + samples)
//   node audit_accessories_misc_wave3.mjs --full    (also prints full list)
...
async function main() {
  ...
  console.log('ACCESSORIES & MISC WAVE-3 DISCOVERY AUDIT');
  console.log('='.repeat(78));
  ...
  console.log('-'.repeat(78));
  console.log('Top 50 leading words, for vocabulary discovery:');
  console.log('-'.repeat(78));
  for (let i = 0; i < Math.min(50, sortedLeads.length); i++) {
  ...
  console.log('');
  console.log('-'.repeat(78));
  const stride = Math.max(1, Math.floor(rows.length / 200));
