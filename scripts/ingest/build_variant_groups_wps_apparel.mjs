#!/usr/bin/env node
/**
 * build_variant_groups_wps_apparel.mjs
 *
 * build_variant_groups.cjs groups WPS products only by shared wps_product_id
 * (Phase 1) -- its name-based fallback (Phase 2) is hardcoded to PU/VTWIN
 * only. For many WPS apparel/helmet lines, WPS's own data assigns a
 * DIFFERENT wps_product_id to every size of the same colorway (confirmed:
 * "Covert X Open-Face Helmet Gloss Black" LG=455069, MD=455070, ...), so
 * Phase 1 can't group them and they're left as 1,797 standalone rows that
 * look like duplicate products in browse.
 *
 * This reuses Phase 2's exact classification logic (attribute extraction,
 * base-name stripping, kit exclusion, pack-qty guard, axis detection --
 * copied verbatim from build_variant_groups.cjs rather than editing that
 * already-tuned script) against WPS rows, scoped to Riding Gear & Apparel
 * only per the approved plan. Groups by (brand, base-name-after-stripping-
 * the-primary-size/color-attribute) -- e.g. all sizes of "Covert X Open-Face
 * Helmet Gloss Black" form one group with Apparel Size as the axis; a
 * different colorway of the same helmet forms its own separate group.
 *
 * Usage:
 *   node scripts/ingest/build_variant_groups_wps_apparel.mjs            # dry run
 *   node scripts/ingest/build_variant_groups_wps_apparel.mjs --apply    # live write
 */
...
const CATEGORY = 'Riding Gear & Apparel';
const MAX_VARIANT_MEMBERS = 20;

// ── String helpers (verbatim from build_variant_groups.cjs) ────────────────
...
function extractAttribute(name) {
  ...
  for (const rule of ATTRIBUTE_RULES) {
    const m = name.match(rule.pattern);
    // `raw` is the literal matched substring (e.g. "SM"), separate from
    // `value`, the normalized/display form (e.g. "S"). Normalization can
    // change the token's spelling (SM->S, MD->M, LARGE->LG) -- stripping
    // the NORMALIZED value out of the original name then silently fails
    // (searching for whole-word "S" never matches inside "SM"), which
    // dropped every SM/MD-sized row out of its group entirely. Stripping
    // must use the raw matched text instead.
...
async function main() {
  const products = await q(`
    SELECT id, name, brand, source_vendor, is_kit, pack_qty
  ...
  console.log(`${products.length} ungrouped active WPS products in "${CATEGORY}"\n`);
  ...
  for (const row of products) {
    ...
    const key = `${row.brand}|${baseName.toLowerCase()}`;
    if (!nameGroups.has(key)) {
      nameGroups.set(key, { baseName, brand: row.brand, members: [] });
    }
  ...
  for (const [, group] of nameGroups) {
    ...
    previewLines.push(`  GROUP [${result.axis}] "${group.baseName}" (${group.brand}) — ${result.members.length} members`);
