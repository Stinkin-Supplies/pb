...
export async function normalizeWps({ batchSize = 500 } = {}) {
  ...
  while (offset < total) {
    ...
    offset += batchSize;
    const pct    = Math.min(offset, total) / total;
    const filled = Math.round(pct * 26);
    const bar    = '█'.repeat(filled) + '░'.repeat(26 - filled);
    const pctStr = (pct * 100).toFixed(1).padStart(5);
    console.log(`[Stage1-WPS] │${bar}│ ${pctStr}% (${Math.min(offset, total)}/${total}) ok ${upserted} err ${failed}`);
  }
