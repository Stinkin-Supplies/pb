...
// ─── SUPABASE CLIENT (replaces Firebase/Firestore) ───────────
// Two clients: browser client (with RLS) and server client (bypasses RLS)
// Use browserClient in React components and API routes for user data
// Use serverClient in workers and admin operations only
...
export function createServerSupabase(cookieStore: ReturnType<typeof import('next/headers').cookies>) {
...
export function subscribeToCart(cartId: string, callback: (cart: unknown) => void) {
  const supabase = createBrowserSupabase()
  return supabase
    .channel(`cart-${cartId}`)
    .on('postgres_changes', {
