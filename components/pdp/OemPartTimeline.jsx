...
// Deduplicate entries by productId — keeps first occurrence
function dedupeByProductId(entries) {
  const seen = new Set();
  ...
  return (
    ...
    >
      <div style={{
        ...
        {src && !imgFailed ? (
          <img
            src={src} alt={entry.name} loading="lazy"
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{
          fontFamily: 'var(--font-bespoke)', fontSize: 12, color: DARK,
        ...
        {entry.packQty > 1 && (
          <div style={{
            fontFamily: 'var(--font-stencil)', fontSize: 9,
            color: MUTED, letterSpacing: '0.04em', marginTop: 1,
          }}>
...
function YearCard({ group, bucket, onClick }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
      <div style={{
        fontFamily: 'var(--font-stencil)', fontSize: 9,
        color: MUTED, letterSpacing: '0.08em', marginBottom: 6,
      }}>
        {group.year}
      </div>
      <button
        ...
        style={{
          width: 112, padding: '10px 12px', borderRadius: 10,
          border: isCurrent ? `1.5px solid ${GOLD}` : `1px solid ${BORDER}`,
          background: isCurrent ? GOLD_L : CREAM,
          cursor: isCurrent ? 'default' : 'pointer', textAlign: 'center',
        }}
      >
        <div style={{
          fontFamily: 'var(--font-stencil)', fontSize: 11, fontWeight: 500,
          color: isCurrent ? '#7a5810' : DARK, marginBottom: 4, letterSpacing: '0.04em',
        }}>
          {group.oemNumber}
        </div>
        <div style={{ fontFamily: 'var(--font-stencil)', fontSize: 9, color: MUTED }}>
          {count} option{count !== 1 ? 's' : ''}
        </div>
        <div style={{
          display: 'inline-block', marginTop: 6,
          padding: '2px 8px', borderRadius: 4,
          fontSize: 9, fontFamily: 'var(--font-stencil)', letterSpacing: '0.06em',
          background: isCurrent ? GOLD_B : bucket === 'older' ? 'rgba(0,0,0,0.04)' : 'rgba(90,138,74,0.12)',
          color: isCurrent ? '#7a5810' : bucket === 'older' ? MUTED : '#5a8a4a',
          border: isCurrent ? `1px solid ${GOLD_B}` : bucket === 'older' ? `1px solid ${BORDER}` : '1px solid rgba(90,138,74,0.22)',
        }}>
...
function Connector() {
  return (
    <div style={{
      ...
      paddingTop: 30, color: BORDER, flexShrink: 0, fontSize: 16,
    }}>→</div>
  );
}
      ...
      <div style={{
        ...
        <div style={{
          ...
          {src && !imgFailed ? (
            ...
            <div style={{
              ...
              color: '#c4b48a', letterSpacing: '0.08em',
            }}>
              NO IMAGE
            </div>
          )}
        ...
        <div style={{ padding: '16px 18px 0' }}>
          <div style={{
            ...
            color: DARK, lineHeight: 1.25, marginBottom: 14,
          }}>
            {entry.name}
          </div>
          {[
            ...
            { label: 'Pack qty',   value: formatPackQty(entry.packQty) },
...
export default function OemPartTimeline({ timeline, currentProductId }) {
  ...
  // Deduplicate by productId — a product with 2 OEM numbers appears twice otherwise
  const leftOptions = dedupeByProductId([...timeline.current, ...timeline.sameYear]);
  ...
  const hasCarousel  = olderGroups.length > 0 || newerGroups.length > 0;
    ...
    <>
      <div style={{
        ...
        <div style={{
          ...
          {/* LEFT: options for current OEM number */}
          <div style={{
            background: CREAM, border: `1px solid ${BORDER}`,
            borderRadius: 10, padding: '14px 12px',
          }}>
            <SectionLabel>Options for {timeline.currentOemNumbers[0]}</SectionLabel>
          ...
          {hasCarousel && (
            <div style={{
              ...
              >
                {olderGroups.map((group, i) => (
                  <div key={group.oemNumber} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                    <YearCard
                      group={group} bucket="older"
                      onClick={() => window.open(`/browse/${group.firstSlug}`, '_blank')}
                    />
                    {(i < olderGroups.length - 1 || currentGroups.length > 0) && <Connector />}
                  </div>
                ))}
                {currentGroups.map((group, i) => (
                  <div key={group.oemNumber} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                    <YearCard group={group} bucket="current" />
                    {(i < currentGroups.length - 1 || newerGroups.length > 0) && <Connector />}
                  </div>
                ))}
                {newerGroups.map((group, i) => (
                  <div key={group.oemNumber} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                    <YearCard
                      group={group} bucket="newer"
                      onClick={() => window.open(`/browse/${group.firstSlug}`, '_blank')}
                    />
              ...
              <div style={{
                fontFamily: 'var(--font-stencil)', fontSize: 9,
                color: MUTED, letterSpacing: '0.06em', marginTop: 6,
              }}>
      ...
      {/* Modal — rendered outside main div so nothing clips it */}
      {modalEntry && (
        <ProductModal entry={modalEntry} onClose={closeModal} />
      )}
    </>
  );
}
