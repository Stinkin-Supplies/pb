...
/**
 * OptionWheel — a draggable/scrollable curved list picker (cover-flow style).
 * Options sit on an arc that curls toward one edge as items move away from
 * the selected position; wheel/touchpad, drag, arrow keys, and click all
 * move the selection. Styling lives in the inline <style> block below
 * (this project keeps CSS colocated in components rather than in separate
 * .css files — see VideoHero.jsx, CategoryPhotoGrid.jsx for the same pattern).
 */
...
const OptionWheel = ({
  ...
  const handleItemClick = useCallback(
    index => {
      ...
      // Fires on every click (unlike onChange, which only fires when the
      // tracked index actually changes) so a click always acts as an
      // immediate "activate" — e.g. navigating, rather than requiring the
      // wheel to already be centered on that item.
