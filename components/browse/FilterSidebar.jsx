...
// Self-contained filter sidebar — collapsible, mobile-aware.
// Props: facets, filters, onChange, open, onClose
// ============================================================
...
// Family → curated sub-model groups
// Each sub-model maps to a display label + array of model_codes to filter by
...
// Families without sub-models — just filter by family name
...
function SidebarSection({ label, isOpen, onToggle, children }) {
  return (
    <div style={{ borderBottom: `1px solid rgba(184,146,42,0.15)` }}>
      ...
      >
        ...
        <motion.span
          ...
          style={{ color: "#bbb", fontSize: "10px", display: "block" }}
        >
          ▼
        </motion.span>
      </button>
  ...
  return (
    <button
      ...
      style={{
        ...
        justifyContent: "space-between",
        width: "100%",
        background: active ? "rgba(184,146,42,0.08)" : "none",
        border: "none",
        padding: "7px 8px",
        cursor: "pointer",
        gap: "8px",
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: "8px", flex: 1, minWidth: 0 }}>
        <div style={{
          width: 10,
          height: 10,
          border: `1px solid ${active ? GOLD : "rgba(184,146,42,0.25)"}`,
...
export default function FilterSidebar({ facets, filters, onChange, open, onClose }) {
  const [collapsed, setCollapsed] = useState(false);
  const [sections, setSections] = useState({ family: true, era: false, category: false, subcategory: false, brand: false, price: false });
    ...
    <>
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: collapsed ? "8px 0" : "8px 0 80px" }}>
          ...
          {[
                ...
                <div style={{ paddingBottom: 8, maxHeight: 320, overflowY: "auto" }}>
                  {HD_FAMILIES_FLAT.map(fam => (
                    <div key={fam}>
                      ...
                      />
                      {/* Sub-models — shown inline when family is selected */}
                          ...
                          {HD_FAMILY_SUBMODELS[fam].map(sub => (
                            <button
                              ...
                              style={{
                                ...
                                gap: 8,
                                width: "100%",
                                background: filters.model === sub.label ? "rgba(184,146,42,0.1)" : "none",
                                border: "none",
                                padding: "5px 8px",
                                cursor: "pointer",
                                borderRadius: 2,
                            ...
                            >
                              <div style={{
                                ...
                                background: filters.model === sub.label ? GOLD : "rgba(184,146,42,0.3)",
                                transition: "background 0.15s",
                              }} />
                              <span style={{
                                fontFamily: "var(--font-stencil, monospace)",
                                fontSize: 9,
                                color: filters.model === sub.label ? DARK : "#888",
                                textTransform: "uppercase",
                                letterSpacing: "0.5px",
                                textAlign: "left",
                              }}>{sub.label}</span>
                  ...
                  {HD_ERAS.map(era => (
                    <button
                      ...
                      style={{
                        ...
                        justifyContent: "space-between",
                        width: "100%",
                        background: filters.era === era.slug ? "rgba(184,146,42,0.08)" : "none",
                        border: "none",
                        padding: "7px 8px",
                        cursor: "pointer",
                        gap: 8,
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: 8, flex: 1 }}>
                        <div style={{
                          ...
                          border: `1px solid ${filters.era === era.slug ? GOLD : "rgba(184,146,42,0.25)"}`,
                          background: filters.era === era.slug ? GOLD : "transparent",
                          transition: "all 0.15s",
                        }} />
                        <span style={{
                          fontFamily: "var(--font-stencil, monospace)",
                          fontSize: 10,
                          color: filters.era === era.slug ? DARK : "#888",
                          textTransform: "uppercase",
                          letterSpacing: "0.5px",
                          textAlign: "left",
                        }}>{era.label}</span>
                      </div>
                      <span style={{ fontFamily: "var(--font-stencil, monospace)", fontSize: 8, color: "#bbb", flexShrink: 0 }}>
                        {era.years}
                      </span>
                    </button>
                  ))}
            ...
            <div key={key} style={{ borderBottom: `1px solid rgba(184,146,42,0.12)` }}>
              <button
                onClick={() => collapsed ? setCollapsed(false) : toggle(key)}
                style={{
                  display: "flex",
                  alignItems: "center",
                  justifyContent: collapsed ? "center" : "space-between",
        ...
        >
          <motion.div
            layout
            style={{ display: "grid", placeContent: "center", width: 20, height: 20 }}
          >
