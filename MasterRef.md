...
| Metric | Value | Status |
|--------|-------|--------|
| catalog_unified (active) | **90,483** | ✅ Verified live session 86 (down from 90,629 — 41 PU/WPS in-store display fixtures deactivated session 75, 2 filler-hose rows deactivated session 85, 9 more soft-deactivated session 86 — 2 Gasket Board sheet-stock items + 7 Dashes & Gauges "REVO ARC Canister Decal" color variants; some natural inventory churn also contributes to the gap, see Known Bugs) |
| **catalog_unified.display_category** | **22 confirmed values, 167 subcategories, 0 NULL** (category count was 2,028 NULL pre-session-74) | ✅ Fixed session 74 — `rebuild_display_category_v2.mjs`; also fixed SADDLEBAGS/TANK structural bugs. **Session 86:** Accessories & Misc + Security & Covers dissolved to 0 rows; Hardware, Covers & General split into Hardware + Accessories & Gear; 0 NULL display_category/display_subcategory anywhere, verified live |
...
| Script | Purpose |
|--------|---------|
| `scripts/ingest/category_cleanup_20260714_part1.sql` through `_part14.sql` | **NEW session 86** — interactive SQL cleanup pass, run live in chat rather than via the usual audit/dry-run/apply `.mjs` cycle. Each part wrapped in its own `BEGIN`/`COMMIT` with a `category_cleanup_backup_20260714[_partN]` snapshot table created first (id-keyed, full pre-change row state) — every single change this session has an explicit one-line rollback path. Dissolved Accessories & Misc + Security & Covers to 0 rows; split Hardware, Covers & General into Hardware + Accessories & Gear; broke Engine's `Engine Parts` (733→29) and `Engine Accessories` (1,041→2) and Windshields & Fairings' `Windshields` (1,242→933) out of undifferentiated mega-buckets into real subcategories; fixed a misfiled Transmission & Clutch "Oil System" (actually engine oil-pump parts) and a "Clutch Cables & Components" subcategory that was 3 different product types. **⚠️ Not wired into `merge_catalog_unified.js`** — see Known Bugs, a future full rebuild will silently undo this session's work unless ported into the ingest pipeline first. Full pass-by-pass detail in HANDOFF_LOG "Session 86" |
...
| `scripts/ingest/audit_wheels_tires_axles_scope.mjs` + `fix_wheels_tires_axles_taxonomy.mjs` | **NEW session 81** — in-place taxonomy cleanup of `Wheels & Tires`. The scoping audit found ZERO cross-category stragglers — the only category in the whole project where that's true. 3,089 rows: 2 subcategory renames (`Tires & Tubes`→`Tires`, `Valves & Balancing`→`Rim Strips, Valve Stems, Valve Stem Cap, Wheel Weights`), 4 unchanged, 330/335 NULL rows classified. Needed tire-brand vocabulary (Metzeler, Firestone, Coker, Dunlop, Michelin, Shinko, Avon) since many tire rows have no generic "tire" word. Two bugs: plural-boundary miss (`\bWHEEL\b` doesn't match "WHEELS"), and vendor-feed mid-word truncation ("...Wide Whitewal") needing a stem match instead of a full-word boundary. 5 rows held back (standalone tools: jump-starter pump, air compressor, reamer/plugger kits) |
| `scripts/ingest/fix_hardware_covers_general_taxonomy.mjs` + `fix_merchandising_taxonomy.mjs` | **NEW session 81** — creates the **Hardware, Covers & General** display_category. 589 rows across 9 subcategories from 15+ source categories. **⚠️ Two structural bugs found, likely relevant to any future classifier**: (1) patterns written as JS `RegExp` objects using `\b`, then `.source` sent to Postgres `~*` as a raw string — **Postgres does not support `\b` at all** (see this doc's own rule below: use `(\s|$)`); every pattern silently matched nothing on round 1 (zero hits across all 9 groupings). Fixed by writing plain Postgres ARE pattern strings directly, no JS-RegExp involved at any point. (2) vendor names use `/` and `-` as word separators ("Holeshot/Brake", "'04-'21") — boundaries widened from `(^|\s)`/`(\s|$)` to `(^|[\s/'-])`/`([\s/'-]|$)`, 67 occurrences in one pass plus a 4th hidden occurrence in a dynamically-built template literal the first bulk-replace missed (only caught by re-verifying actual file content). Exclusion list grew to ~50 terms across 7 dry-run rounds. Found the `CLUTCHES?` variant of the trailing-S bug (matches plural, silently never matches the much-more-common bare singular "Clutch") — same bug FAMILY as Dashes & Gauges' `\y`/`\b` mixup and this session's Wheels & Tires plural miss, but a distinct manifestation each time; worth treating as a recurring class of error, not a one-off. Merchandising subcategory (156 rows) added same-session after Laken corrected the original vocabulary guess (retail display fixtures → patches/stickers/gift sets/keychains); confirmed bare "V-TWIN" and bare "PIN" are both noise-dominated (95%+ false-positive) and excluded as signals entirely, not just narrowed |
| `scripts/ingest/audit_chopper_supplies_scope.mjs` / `_v2.mjs` | **NEW session 81, audit only, not applied** — round 1 (broad vocabulary) found "Spool" mostly means dirt-track spool-hub wheels not wire spools, and "chopper"/"hardtail"/"springer"/"ape hanger"/"sissy bar" matched 1,669 rows that are almost entirely existing, correctly-placed inventory elsewhere. Laken's scope correction: Chopper Supplies = ONLY genuine bulk consumables (wire, paint, raw stock), NOT chopper-build components. `_v2.mjs` narrows vocabulary accordingly but has not been run yet |
| `scripts/ingest/audit_full_catalog_health.mjs` | **NEW session 81** — read-only, catalog-wide health check requested by Laken: per-category row counts, NULL-subcategory counts/percentages, subcategory breakdowns, straggler samples. First time this project has had a single consolidated view of classification state across all 26 categories at once, rather than per-category figures scattered across sessions. Confirmed 90,609 active rows, 6,859 NULL subcategory (7.6%) catalog-wide, heavily concentrated: Accessories & Misc (3,203, 80.6%), Riding Gear & Apparel (1,760, 41.9%), Fenders & Body (98, 79.0% of a small category), Tools & Chemicals (547, 29.9%), Foot Controls (454, 14.4%), Frame & Hardware (436, 18.3%), Suspension (99, 18.9%), Security & Covers (36, 17.0%), Seating (142, 3.8%) |
| `scripts/ingest/audit_accessories_misc_nulls.mjs` / `_crossclassify.mjs` / `fix_accessories_misc_taxonomy.mjs` | **NEW session 81, drafted + regression-tested, NOT yet run against live data** — Accessories & Misc's 3,203 NULL rows (80.6%) confirmed to be a genuine cross-CATEGORY misplacement problem, not a missing-subcategory problem like every prior category. 886/3,203 (28%) already coverable by extending the Hardware/Covers, Wheels & Tires, and Merchandising vocabulary built earlier this same session. All 43 "Spool" rows individually verified by name (not assumed from the word alone, per Laken's explicit instruction, twice this session): 42 are spool-hub wheels/hubs/axles, 1 (`Parkerized Spool Shifter Peg`, caught by Laken personally) is a Foot Controls shifter peg where "spool" describes shape, not a wheel — handled by explicit ID override. New rules added for parts misplaced at the category level entirely (Starter Relay/Ignition Coil→Electrical, Grip Set→Handlebar & Controls, Shifter Peg→Foot Controls, Shifter Fork/Gear→Transmission & Clutch, Valve Stem Cover→Wheels & Tires, Damper Washer→Suspension). 36-case regression suite passed. **The actual dry run against the live database has not been executed — first task for next session** |
| `scripts/ingest/fix_foot_controls_taxonomy.mjs` | **NEW session 80** — in-place subcategory rename/consolidation of `Foot Controls`, not a keyword rebuild (the 8 existing subcategories were already clean). Maps by existing `display_subcategory` VALUE, not by re-parsing product names: Forward Controls→Forward Controls & HW, Rearsets & Mid Controls→Mid-Controls, Floorboards→Floorboards & HW, Footpegs+Shifters (folded)→Footpegs, Shift Pegs, & HW. 2,282 rows renamed. Confirmed idempotent (safe to re-run). `Kickstands`/`Highway Bars & Pegs` and NULL-subcategory rows deliberately untouched |
| `scripts/ingest/fix_cables_stragglers.mjs` | **NEW session 80** — Cables straggler sweep, NOT a fresh build (Cables already existed with 4,253 correct rows from a prior session). Pulls from 9 other categories, with explicit routing for grip products (→ Handlebar & Controls/Grips) and battery cables (→ Electrical/Batteries) rather than sweeping them into Cables. Required an ordering fix: specific cable-product rules → other-part routing (throttle assembly/handlebar/lever/throttle clamp) → generic bare-"cable" fallback, in that order, or the fallback swallows non-cable rows first. **⚠️ 307 rows applied into `Cables/Universal, Build Your Own` are known-misrouted** (grip/throttle-sleeve products with pattern gaps) — needs a follow-up correction pass |
| `scripts/ingest/fix_frames_suspension_taxonomy.mjs` | **NEW session 80** — creates the **Frames & Suspension** display_category (genuine new third category — `Frame & Hardware` and `Suspension` both remain live, untouched). 3,390 rows across 7 subcategories. Required an exclusion list (BRAKE/ROTOR/ENGINE CASE/CAM/SPROCKET/TRANSMISSION/CLUTCH/etc, plus DOCKING/SHIFTER/STARTER/GEAR found in later rounds) to stop bare HARDWARE/SPRING/FORK/SHOCK fallback patterns from sweeping in cross-system fasteners that happen to sit in `Frame & Hardware`'s shared "Hardware & Fasteners" bin. Settled at 52.5% coverage by design — remaining rows are correctly-excluded, not a gap |
| `scripts/ingest/fix_dashes_gauges_taxonomy.mjs` | **NEW session 80** — in-place rebuild of `Instrumentation` into **Dashes & Gauges**, pulling from `Instrumentation` (full) + `Fenders & Body`/`Accessories & Misc`/`Handlebar & Controls` stragglers. 1,340 rows, 7 subcategories. Round-1 bug: `\y` (Postgres word-boundary syntax) used inside JS regex literals instead of `\b` — JS doesn't recognize `\y`, silently broke every boundary rule (33.5% coverage until fixed) |
...
5. CategoryBentoGrid (19 categories — excludes Riding Gear & Tools)
   - ⚠️ **STILL NOT UPDATED.** `display_category` is now **26** values as of session 81 (Hardware, Covers & General added as genuine net-new, on top of session 80's Frames & Suspension addition; Wheels & Tires, Dashes & Gauges, and Footrests & Floorboards are in-place rebuilds, not new categories, so they don't add to the count on their own — this note has now been stale for three sessions running).
...
| **ASYMMETRIC PATCHING — a word-order/adjacency fix on one side of a symmetric pair doesn't imply the mirror side got it too.** Tanks & Body's dry-run 3 fixed `FUEL PRESSURE REGULATOR` (interposed word breaking a fuel/regulator adjacency match) but never applied the identical fix to `FILTER OIL`/`COOLER OIL` on the oil side — that gap survived two more dry-run rounds before being caught (session 79) | Whenever a fuel/oil, front/rear, or similar paired-vocabulary bug is found and fixed on one side, explicitly check the other side's equivalent rule in the same edit, don't assume symmetry was already handled | Same underlying regex-adjacency root cause as the trailing-S and BRAKE-adjacency bug families — but the new failure mode here is fixing one instance of the pattern and forgetting its sibling exists, not failing to spot the pattern at all |
| **A CATEGORY-LEVEL MIGRATION'S TRUE SCOPE CAN EXCEED THE NAME USED TO INTRODUCE IT.** The queue item named "Tanks & Oil Filters" was delivered, per Laken's own written spec, as a 4-source migration spanning gas tanks/caps, carb fuel valves/filters, EFI fuel lines/regulators, oil tank/dipstick/hoses, oil filters/mounts, fuel/oil line clamps, front/rear fenders, fender trim, AND license plates — not the two-domain scope the short name implied (session 79) | Audit against the full spec text the person actually writes out, not the short name used to introduce or queue the work | Also surfaced an unplanned discovery mid-audit: an entire windshield product line (117 rows) sitting unmigrated in the category being retired, requiring a reroute to a third, pre-existing category (`Windshields & Fairings`) that wasn't part of the original scope conversation at all |
| **SIBLING-PATTERN SIMILARITY IS NOT CONFIRMED IDENTITY.** 20 ANTHEM/RACE LEVERS/SHORTY MX SKUs matched the exact naming pattern of 180+ already-classified `Brake Hardware` siblings (same brand, same phrasing, differing only by newer model-year fitment) — proposed auto-classifying them the same way (session 79) | Laken explicitly declined; left NULL, flagged genuinely ambiguous rather than pattern-matched | "Matches the pattern that 180 confirmed-correct siblings share" and "is itself confirmed correct" are different claims — a large, confident-looking sibling cluster is not by itself grounds to skip the individual judgment call, especially right after the adjacent "don't force a fallback bucket" lesson reinforced holding back rather than guessing |
...
*Master Reference — Last updated July 14, 2026 · Session 85 (see HANDOFF_LOG.md "Session 85" for full session detail)*
