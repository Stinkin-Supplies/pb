...
Classified all 4,550 rows from scratch (old subcat boundaries didn't map onto the new 15). Confirmed and expanded Laken's spot-check list by full-category pattern sweeps: **57 rows moved out of the category entirely** — 3 belt buckles → Accessories & Gear/Merchandising, 1 Linkert parts book → Accessories & Gear/Shop Books, 20 Feuling "Vented Dipstick"/oil-tank-breather rows → Tanks & Body/Oil Tank & Dipsticks, 13 oil-line-fitting rows → Tanks & Body/Oil & Fuel Lines, 5 engine-crankcase/rocker-box breather rows (distinct from air-cleaner breathers) → Engine/Engine Accessories, 3 Loctite Threadlocker rows → Tools & Chemicals/Chemicals & Lubricants, 2 Points & Condenser kits → Electrical/Points, Distributors & Accessories, 28 O2/MAP/throttle-position sensor and barometric-switch rows → Electrical/Sensors & Switches (Laken's call, over keeping them in a "Modules" bucket here). Laken's own "not sure yet" item, 10 MOROSO "Air Oil Separator" rows, stayed in-category per her decision, filed under General.

Two whole missed clusters surfaced only by sampling the leftover pile, not from Laken's original list: **138 "Air Intake Kit" rows** (Vance & Hines VO2, Cobra RPT/PowrFlo, K&N Aircharger/Street Metal) — functionally identical to Air Cleaner kits but worded differently, and a large body of abbreviated **"AIRCLNR"/"AIRCLEANER"** (no space) product codes from RSD and ROUGH CRAFTS that a literal `air cleaner` match missed. Also folded in 61 unbranded **Linkert** vintage-carburetor small parts (bowl assemblies, venturis, throttle kits, air snoots) that don't say "carburetor" anywhere in the name — recognized by brand alone. General landed at a clean 121 rows (2.7%) after these fixes, confirmed via a full manual read of the leftover pile before accepting it as final (Laken's call, rather than accepting the first-pass 511-row/11% General bucket sight unseen).

**Final 15-bucket counts**: Air Cleaner 1,360 / Jets, Needles, Screws etc. 1,017 / Carburetor 551 / Air Filter 452 / Manifold 209 / Modules 187 / Air Cleaner Mounts, Brackets & Hardware 165 / General 137 / Fuel Filters 112 / Air Cleaner Adapters 87 / Rebuild Kits 83 / Backing Plates 71 / Breather Bolts 16 / Throttle Body 14 / Flange 14.

Applied the standing >150-row detail-grouping policy to the 7 qualifying buckets in the same pass (`scripts/ingest/detail_fuel_air_carbs.mjs`, 3,941 rows tagged, 0 stragglers in any bucket — each ruleset ends in a catch-all fallback labeled for what it actually is, e.g. "Complete Air Cleaner Kits & Assemblies", rather than leaving unclassified rows). Added a `SUBCATEGORY_DISPLAY_GROUPS` entry to `lib/db/browse.ts` for the new category name. Reindexed (90,483 docs, 0 errors, Typesense count matches Postgres exactly) and verified live in the Browser pane: category rename, alphabetized subcategory sidebar, DETAIL tier auto-populating, and the Tanks & Body oil-tank moves all confirmed on the actual site. See `scripts/ingest/rebuild_fuel_air_carbs_taxonomy.mjs` (rename + 15-bucket split + out-of-category moves, backup table `backup_fuel_air_carbs_20260717`) and `detail_fuel_air_carbs.mjs`.

**Process note**: for this rebuild, presented the full mapping proposal (bucket counts + the misfiled-item findings + specific ambiguous calls — air/oil separators, EFI sensors, General bucket size) to Laken via structured questions *before* writing anything, rather than deciding the judgment calls unilaterally. All four calls came back with a clear answer (including one custom instruction — route the parts book specifically to Shop Books, not the generic Merchandising bucket used for the belt buckles) before any SQL ran.

---

## Session 90 (July 17 2026) — Retroactive "General bucket policy" applied to the three categories finished before the policy existed: Saddlebags/Sissy Bars & Luggage, Gaskets & Seals, Seating

Closed out the standing retroactive-work item from session 89 (see `feedback-general-bucket-policy` memory): detail-groupings for every bucket over ~150 rows, catch-all renames, and `SUBCATEGORY_DISPLAY_GROUPS` filter-sidebar entries, applied to the three categories that were completed before that policy existed. All three verified live in the Browser pane after a full Typesense reindex (90,483 docs, 0 errors, row count unchanged throughout).

**Saddlebags, Sissy Bars & Luggage** (`scripts/ingest/rebuild_luggage_detail_groups.mjs`): renamed the 95-row "General" catch-all → **Bags, Packs & Duffels** (sampled content: almost entirely tank bags/tail bags/duffels/backpacks, not saddlebag-specific). Added detail groups to the 5 buckets over 150 rows — Sissy Bars/Sideplates & Hardware, Saddlebags/Lids & Covers, Sissy Bar Pads & Bags, Luggage Racks, Saddlebag Latches/Mounts & Hardware — 1,423 rows tagged, coverage 85–100% per bucket.

**Gaskets & Seals** (`scripts/ingest/rebuild_gaskets_detail_groups.mjs`): no catch-all bucket existed (session 88's rebuild was already clean), so this was pure detail-grouping on the 5 buckets over 150 rows — Engine, Transmission, Forks, Carbs, Electric & Lighting — 3,748 rows tagged. **One correction found while sampling**: 10 Kibblewhite "Valve Guide Seals" rows were sitting in Electric & Lighting (they're engine valve components, not electrical parts) — moved to Engine, tagged into the new Valve Guide & Valve Stem Gaskets detail group.

**Seating** (`scripts/ingest/rebuild_seating_detail_groups.mjs`): investigated whether the 185-row "Seats" bucket (not one of Laken's 12 named model-platform/part-type buckets) was itself an unlabeled catch-all — it was. Found 20 V-Twin "Seat Post Assembly"/"Seat T" rows that are hardtail/springer *mounting hardware*, not seats (moved to Seating Hardware), and 6 Wild Ass "CUSHION..." rows that are add-on cushions, not seats (moved to Accessories). Renamed the remainder → **Universal & Styled Seats** (brand-styled/universal-fit seats not tied to one platform). Added detail groups to all 7 buckets over 150 rows (the 4 model-platform buckets share one ruleset — Pillion Pads/Solo/2-Up & Passenger/w-Backrest/Other Complete — since they all follow the same seat-type naming convention; Solo Seats and Seating Hardware each needed their own ruleset) — 2,930 rows tagged.

**`SUBCATEGORY_DISPLAY_GROUPS` in `lib/db/browse.ts`**: added entries for all three categories, alongside the existing Handlebars & Hand Controls one. Saddlebags/Luggage and Gaskets & Seals have no non-type leftover bucket, so their "type group" is just the full alphabetized subcategory list. Seating's type group is the platform/usage buckets (Chopper/Rigid/Buddy, Dyna, FXR, Pillion Pads, Softail, Solo Seats, Sportster, Touring, Touring Back Rest, Universal & Styled Seats); the 3 support buckets (Accessories, Seat Knobs/Screws, Seating Hardware) sort alphabetically after. Verified live: filter sidebar shows the correct grouped order for all three, and the DETAIL tier auto-populates correctly when a large subcategory is selected (e.g. Gaskets & Seals → Engine shows Rocker Box & Pushrod/Complete & Top End Kits/Head Gaskets/... in the sidebar).

---

## Session 89 (July 17 2026) — Handlebars & Hand Controls and Saddlebags/Sissy Bars & Luggage remapped onto finalized specs; new "General bucket" standing policy established; full Tools & Chemicals audit

**Handlebars & Hand Controls** rebuilt per Laken's finalized spec. The residual ~1,500 rows needed a brand-by-brand walkthrough with Laken because several vendors' naming doesn't map onto generic ape/Z/T/drag keywords at all (Fat Baggers "Flat/Pointed/Round Top", LA Choppers "Kage Fighter"/"Thresher"/"Twin Peaks", Burly Brand's named lines) — two new subcats, **Moto Style** and **Levers & Hand Controls**, were added beyond Laken's original list during that walkthrough. **Bug caught post-apply by Laken spot-checking the live site** ("we saw a lot of errors, replica bars inside, drag bars inside"): whole-brand catch-all fallback rules (e.g. "everything else MCM → Ape Hangers") were being checked *before* the specific style keywords instead of after, silently swallowing 151 rows that explicitly said "Z Handlebar"/"Replica Handlebar"/"T-Bar"/"Drag" into Ape Hangers. Same "spot-check catches the bug" pattern as session 87 pass 11. Fixed both the live data (targeted correction script) and the root cause — `classify()` in `scripts/ingest/rebuild_handlebars_taxonomy.mjs` now checks brand-specific `BRAND_OVERRIDES` first, keyword rules second, and whole-brand `BRAND_FALLBACKS` last.

Follow-up fixes caught the same way, same session: **Burly wiring kits** — 219 "CNTRL KIT" rows moved Ape Hangers → Handlebar Switches & Wiring Kits. **Twist-grip/throttle assemblies** — 41 items moved out of Grips into the new Hardware & Accessories detail group "Throttle Components" (regex needed a negative lookbehind, `(?<!w\/o )throttle sleeve`, to stop matching HardDrive grips explicitly labeled "W/O Throttle Sleeve"). **Beach bars** — 16 rows misfiled in Drag Style Bars moved to the new Plain Handlebar subcat. **Risers** — 31 Drag Specialties "Big Buffalo"/Wild 1 "Chubby" riser products misfiled in Drag Style Bars moved to Risers & Clamps (word-order bug: a pull-out rule was matching "Riser" regardless of whether it appeared before "Handlebar," meaning a complete bar, e.g. "Kage Fighter Welded Straight-Riser Handlebar," vs. after, meaning a riser accessory — fixed by requiring `riser` NOT immediately followed by `\s*handlebar`). **Levers & Hand Controls** (910 rows, Laken: "lots of hardware inside the levers and hand controls") — split into 4 detail groups: Levers (576), Master Cylinders (155), Handlebar Control Kits (134), Hardware & Small Parts (45), zero stragglers. **Plain Handlebar** subcat created per Laken's request ("can we build a Plain Handlebar sub cat to put the indeterminable options") — 307 rows pulled out of the Hardware & Accessories "Handlebars" detail group. Also 2 stray MCM items ("Throttle by Wire Handlebar Plug", "1 inch Handlebar Sleeve Raw Steel") moved out of Ape Hangers.

**Hardware & Accessories** — Laken's catch-all "General & Accessories" bucket renamed and given real structure: 6 detail groups (Handlebars, Throttle Components, Handguards, Phone & Device Mounts, Hardware & Fasteners, Stash Tubes). Two cross-subcategory moves surfaced during classification: ~19 "Handlebar Control Kit" items → Levers & Hand Controls, ~12 ODI "Vans Signature TBW" items → Grips. One regex false-positive caught before applying: Emgo "Clip-On Handlebar" (a complete bar style) was matching the bare `\bclip\b` Hardware & Fasteners rule — fixed with `\bclip\b(?!-on)`.

**New standing policy, established this session** (see `feedback-general-bucket-policy` memory): (1) any subcategory bucket over ~150 items gets `display_subcategory_detail` groupings; (2) catch-all "General" buckets get renamed category-specifically, never left as an undifferentiated dump; (3) the subcategory facet list itself gets alphabetized/type-grouped via a new `SUBCATEGORY_DISPLAY_GROUPS` config + `applySubcategoryGrouping()` function in `lib/db/browse.ts` (opt-in per `display_category`, wired into the `facets.subcategories` return; verified live against Handlebars & Hand Controls, confirmed Engine unaffected). **Retroactive work still open**: Gaskets & Seals, Seating, and Saddlebags/Sissy Bars & Luggage's own General bucket were all completed before this policy existed and still need it applied. Tools & Chemicals (now 6 subcats, see below) is also a `SUBCATEGORY_DISPLAY_GROUPS` candidate, not yet added.

**Saddlebags, Sissy Bars & Luggage** rebuilt per Laken's finalized spec (`scripts/ingest/rebuild_luggage_taxonomy.mjs`).

**Tools & Chemicals — full audit**, kicked off by Laken noticing on-bike mounted toolboxes (bolt to frame/fender) sitting in the Tools subcategory. Full read of all 1,001 rows (not a keyword sample) found ~70 toolbox/mounting-hardware items (Colony/V-Twin/Wyatt Gatling), one more hiding in Chemicals & Lubricants — all moved to a **new subcategory, "Tool Boxes & Mounts," under Saddlebags, Sissy Bars & Luggage** (cross-category move; soft tool rolls/bags deliberately left in Tools per Laken's call — different product type). Same full read caught ~30 genuine hand tools misfiled into Chemicals & Lubricants (oil filter wrenches, drain tools, grease guns, foam cannons — mostly because "oil" appears in both tool names and chemical names) and 2 rows of a $3,850+ Jims transmission gear kit that had drifted out of Transmission & Clutch > Mainshaft & Components (6 sibling rows were already correctly filed there).

Then, applying the new >150-item policy to the now-clean Tools (968) and Chemicals & Lubricants (758) buckets: built keyword clusters, presented sizes to Laken ("i want the details on those huge groups to find if we have enough to make more"), and **promoted 4 to full standalone subcategories** per Laken's "all": Hand Tools & Sets (240), Engine & Drivetrain Tools (259), Engine & Motor Oil (185), Cleaners Wash & Detailing (145). Everything else got `display_subcategory_detail` tags (11 groups in Tools, 13 in Chemicals & Lubricants), leaving 153 Tools and 68 Chemicals genuine stragglers ungrouped. See `scripts/ingest/rebuild_tools_chemicals_taxonomy.mjs`.

**Bug caught immediately after, same session**: the two newly-promoted subcats' bare keyword rules collided with unrelated products — "CAM LOCK ULTRA TIE-DOWN" (HIGH ROLLER) matched Engine & Drivetrain Tools' `cam.*lock` pattern; "RATCHET ULTRA TIE-DOWN" (HIGH ROLLER/PowerTye/FIRE POWER) and 6 Dowco "COVER WEATHERALL PLUS RATCHET ATTACHMENT" motorcycle covers matched Hand Tools & Sets' bare `ratchet` pattern. Laken's literal ask was "cargo nets should go in luggage, tie downs should do the same" — before executing, checked for an existing better-matching destination and found one: **`Accessories & Gear > Towing Equipment`** already had 200+ rows of the same product type (PowerTye/Drag Specialties/Emgo/ERICKSON cargo nets and tie-downs, Canyon Dancer bar harnesses, wheel chocks, trailer hitches), plus **`Accessories & Gear > Motorcycle Covers`** (49 rows) for the Dowco covers. Surfaced this to Laken, got confirmation, redirected there instead of creating a second scattered luggage pocket. 24 cargo-net/tie-down rows + 6 cover rows moved; reindexed clean. See `scripts/ingest/fix_tools_cargo_tiedowns_covers.mjs`.

**Recurring meta-lesson, reconfirmed a third time this session**: bare/generic keywords (`cam`+`lock`, `ratchet`, and this session's brand-fallback-ordering bug) in a broad classifier will always catch unrelated products sharing vocabulary with the target domain — same root cause as session 87's "Housing" disaster. Every new keyword rule, especially ones added late in a large pass, needs a full-result-set sanity sweep before the job is called done, not just review of the buckets it was designed to catch.

**Housekeeping**: accidentally overwrote and `rm -f`'d a pre-existing tracked file, `scripts/ingest/audit_tools_chemicals.mjs` (an earlier session's NULL-subcategory audit), by reusing its filename for a fresh one-off audit — twice in this session. Caught both times via `git status` showing a `D` flag, restored via `git checkout HEAD --` both times. Lesson: check for a filename collision before naming a throwaway audit script.

---

## Session 88 (July 17 2026) — Gaskets & Seals rebuilt using the H-D OEM parts-catalog section chart as ground truth; Seating fixes

**Gaskets & Seals** (4,250 rows) rebuilt into 7 functional subcategories — Exhaust, Forks, Engine, Transmission, Electric & Lighting, Brakes, Carbs — using a hybrid classifier. Laken supplied the actual Harley-Davidson OEM parts-catalog section-number chart as ground truth: James Gasket reuses bare HD section numbers as its own part numbers, and the `oem_numbers[]` column carries cross-referenced HD numbers for other brands, so the chart's section number was used as the primary classification signal, with keyword rules as fallback/override where the chart's coarser ranges (11–18, 25, 83) proved internally mixed on sampling. See `scripts/ingest/rebuild_gaskets_seals_taxonomy_v2.mjs`.

**Seating** — 3 fixes: seats that had been misfiled into the hardware bucket moved back to Seating; buddy seats consolidated into a renamed **Chopper/Rigid/Buddy** subcategory; sissy pads moved to Luggage & Racks. See `scripts/ingest/session88_seating_fixes.mjs`.

---

## Session 87 (July 15 2026) — "Full and final audit" kicked off: Windshields & Fairings, Dashes & Gauges, and Tanks & Body remapped onto Laken's finalized subcategory specs; stale admin-panel category dropdown fixed

**New phase of the project** — Laken is now handing over finished top-level subcategory specs (exact names, no more open-ended classification) one category at a time, starting with these three. Method: audit current live breakdown → sample real product names for any bucket that doesn't map 1:1 onto the new spec → propose full mapping with counts → Laken's calls on the ambiguous ones → apply (`category_cleanup_20260715_part1.sql` through `_part7.sql`, same BEGIN/COMMIT + backup-table-per-part convention as session 86) → verify → Typesense reindex. All 7 parts committed clean, 0 errors. **90,483 active products unchanged** (no rows lost), reindexed to 90,483 Typesense docs, 0 errors.

### Admin `?admin=1` inline-edit panel — stale hardcoded category list fixed first
Laken flagged that the PDP inline editor wasn't showing all the correct categories/subcategories. Root cause: `components/admin/AdminEditPanel.jsx` hardcoded a 20-category list dating from well before this project's taxonomy work — still had `Instrumentation`/`Frame & Hardware`/`Suspension`/`Security & Covers`/`Accessories & Misc` (all retired or dissolved by session 86), missing everything created since (`Tanks & Body`, `Dashes & Gauges`, `Frames & Suspension`, `Cables`, `Gaskets & Seals`, `Windshields & Fairings`, `Hardware`, `Accessories & Gear`). This is the same "frontend hardcoded category array" bug flagged every session since 77, just in a different file than the browse-page one that was already fixed. **Fixed properly, not just patched**: new endpoint `app/api/admin/catalog/categories/route.ts` (GET, no auth — same aggregate data the public `/categories` page already exposes) returns the live `display_category` → `display_subcategory` breakdown straight from `catalog_unified`; `AdminEditPanel.jsx` now fetches it on mount instead of hardcoding, so it can't go stale again after a future merge/split. Verified live in the Browser pane against a real product — all 22 current categories present, retired ones gone, 0 console errors.
**Left alone on purpose**: `app/admin/catalog/CatalogTable.jsx` has its own separate stale `CATEGORIES` array (edits the raw `category` column via a different, Supabase-auth-gated legacy bulk tool, not `display_category`). Laken's call: leave it stale unless that page actually becomes a real bulk-editing tool.

### Windshields & Fairings — 1,453 rows, clean 7→5 subcat remap, zero content ambiguity
Straight rename/merge, no row-level judgment calls needed: Windshields (933) and Fairings (179) unchanged; Windshield Trim (59) + Windshield & Fairing Bags (12) → **Trim, Windshield Bag & Accessories** (71); Windshield Hardware (212) → **Mounts, Brackets & Hardware** (211 final — 1-row drift, almost certainly a stock-driven `is_active` flip on the live catalog between audit and apply, not a logic bug); Lower Fairings (39) + Deflector Screens (19) → **Lowers & Deflectors** (58).

### Dashes & Gauges — 1,345 rows, Speedometers/Dash & Panel/hardware-buckets were clean, but "Gauges" (374) was a genuine mixed pile
Speedometers (453) unchanged. Dash & Panel (268) → **Tank Dashes/ Console & Accessories** (confirmed clean on inspection — dash inserts/panels/covers, plus Fuel Tank Console Doors per the session-80 precedent to keep those here, not in Tanks & Body). Housings (128) + Gauge Hardware (74) + Instrument Hardware (48) merged into **Mounts, Housing & Hardware**. The 374-row "Gauges" bucket needed real classification, not a rename — random sampling showed it was instruments tangled with mounting hardware (gauge mounts/brackets/bezels/adapter rings/tachometer covers) and several false positives (wire-gauge/AWG wiring-terminal items using "gauge" to mean wire thickness; gauge-branded diagnostic tools like "Jims Shifter Fork Gauge Tool"; a tire pressure gauge). Two refinement rounds on the classifier (first pass caught the wire/tool false positives were still leaking into "Misc Gauges") landed on: **Mounts, Housing & Hardware** +143 (total 393) for mount/bracket/adapter/clamp/riser/cover items plus standalone bezel covers (excluding actual multi-gauge instrument kits where "bezel" is just a color variant, e.g. "MLX 4-Gauge Instrument Kit - Chrome Bezel" correctly stayed a gauge product); **Tachometer** (39) for tach/rpm-named instruments; **Misc Gauges** (167) for the real remaining oil-pressure/fuel/temp/voltmeter instruments; 13 rows pulled entirely out to **Electrical/Wiring & Components** (wire-gauge false positives); 12 rows pulled out to **Tools & Chemicals/Tools** (gauge-tool and tire-pressure-gauge false positives).

### Tanks & Body — 4,239 rows, biggest of the three, three real problems found by inspection
Gas Tanks & Gas Caps (1,387) unchanged. Rear Fender wording tightened, License Plate bucket renamed to **License Plates** (confirmed on inspection this deliberately keeps combo taillight/license-light fixtures bundled in, per Laken's own session-79 call not to split by fixture type), Oil Filters bucket renamed. Three buckets needed real work:
1. **"Oil Tank, Dipstick, Hoses" (553)** was two product types: genuine tanks/dipsticks (283, stays **Oil Tank & Dipsticks**) vs. oil lines/hoses/fittings (213, merged into new **Oil & Fuel Lines**) vs. **57 oil coolers that don't fit any of the 8 named subcategories at all** — Laken's call: move out of Tanks & Body entirely into **Engine/Cooling System** (joining that bucket's existing 18 rows, now 75 total).
2. **"Fuel Lines, Regulators, Filters (EFI)" (297) + "Fuel/Oil Line, Clamps and Finishers" (229)** merged into the same new **Oil & Fuel Lines** bucket (now 737 total combined with the oil-tank-bucket's line/hose portion) — except 2 misfiled "Spring Fork Fender ... Mount Clamp Set" rows that are fender-mounting hardware, not fuel/oil line parts, routed to Front Fender & Hardware instead.
3. **"Fender Parts & Accessories" (360)** — only 41 rows had an obvious "front"/"rear" word; Laken flagged that vendor titles sometimes use bare "FR"/"RR" abbreviations instead, which a follow-up regex confirmed (9 more rows: "FENDER FR 4.506-17 FXDWG", "FENDER RR 7.5" STREET BOB", etc.). Final split: front-labeled (17 incl. FR) → **Front Fender & Hardware** (504→523); rear-labeled (33 incl. RR) → **Rear Fender, Struts & Hardware** (435→468); 4 "Boom Mat" sound-deadening items (not fender parts at all) → **Tools & Chemicals/Tools**; the remaining 306 generic fender accessories (skins, bibs, risers, mud flaps, eliminators, spacers — no front/rear signal at all) became the new **General** bucket, the first time this project has used a deliberate miscellaneous catch-all as a *named, permanent* subcategory rather than a temporary NULL-cleanup bin.

**Final Tanks & Body**: Gas Tanks & Gas Caps 1,387 / Oil & Fuel Lines 737 / Front Fender & Hardware 523 / Rear Fender, Struts & Hardware 468 / License Plates 368 / General 306 / Oil Tank & Dipsticks 283 / Oil Filters 106 — exactly Laken's 8 named subcategories, 0 leftover/unmapped buckets.

### Follow-up passes 8 & 9 — same session, caught by Laken spot-checking the live site after apply
Laken flagged "there is a ton of hardware inside the speedometer subcat" — full review of "Speedometers" (453) found the exact same instrument-vs-mounting-hardware tangle as the "Gauges" bucket (part 3): speedometer mounts/relocation brackets/adapter rings/clamps/covers/visors/block-off plates mixed in with actual speedometer heads and drive/sensor components. Split 84 hardware items out to **Mounts, Housing & Hardware** (now 477), leaving a clean 369 in **Speedometers**. Drive/sensor components (ring gear, drive unit, sensor, sender, reset knob) were kept bundled with the instruments rather than moved to hardware — same rule used for gauge senders in part 3.

Laken then separately flagged 8 dipstick/oil-plug items still sitting in "Misc Gauges" (they have an integrated temperature gauge, which is why the earlier regex classifier caught them as gauge products) — moved by explicit ID to **Tanks & Body / Oil Tank & Dipsticks** (284→292): `DIGITAL OIL DIPSTICK BLACK/SILVER FITS 07-16 FLH/FLT`, `Oil Dipstick Gauge - Black/Silver - Touring/Trike '17-'25`, `Oil Plug - With Temperature Gauge - 2.87"/3.87"`, `OIL TEMP GAUGE 2 3/4" DIPSTICK BLACK FACE`, `OIL TEMP GAUGE 2 7/8" DIPSTICK 00-17 SOFTAIL`. Applied via `category_cleanup_20260715_part8.sql` and `_part9.sql`, same backup-table + BEGIN/COMMIT convention. **Lesson repeated from session 84**: "everything's done" after an apply pass should still be treated as provisional until Laken (or a fresh sample pull) actually looks at the result — both of these were caught by spot-checking the live site after the main 7-part pass was declared complete, not by the audit itself.

### Discovered/used: the site already has a built-in tier-3 filter, just unpopulated for most subcategories
Laken asked how to filter past subcategory (e.g. "vented gas caps, non-vented gas caps, gas tank hardware" within the 1,387-row Gas Tanks & Gas Caps bucket). Turned out the **entire stack already exists and was built in an earlier session**: a `display_subcategory_detail` column, full filter/facet plumbing in `lib/db/browse.ts` (`BrowseFilters.subcategoryDetail`, a `subcategoryDetails` facet query, even a `detail_priority` sort rule), and a "DETAIL" section in `components/browse/FilterSidebar.jsx` that auto-renders whenever a subcategory has non-null detail values — it just auto-appears/disappears per subcategory, no frontend work needed. The actual gap was data: only ~38k/90k active rows have any tier-3 value at all, and Gas Tanks & Gas Caps had none. There's also a dedicated existing tool for populating it, `scripts/ingest/rebuild_subcategory_detail.mjs` (shadow-column-then-promote pattern, `display_subcategory_detail_v2` → `display_subcategory_detail`, used for 15 other oversized subcategories previously). Added a `'Tanks & Body|Gas Tanks & Gas Caps'` classifier to it (**caught and fixed the project's recurring Postgres-`\b` word-boundary bug again**, this time inside a plain JS regex where `\b` actually works fine — no bug there, but worth remembering the pattern shows up in SQL contexts specifically) and ran `--write-shadow --promote`: 1,379/1,383 rows classified (99.7%) into **Gas Tanks** (410), **Gas Tank Hardware** (249), **Fuel Petcocks** (246), **Gas Tank Trim & Emblems** (170), **Vented Gas Caps** (120), **Gas Caps (Other)** (96), **Non-Vented Gas Caps** (88). 4 true leftovers unclassified on purpose, incl. 2 "Quickbob Rear Fender" rows that are genuinely misfiled into this subcategory entirely (a subcategory-level fix, not tier-3 — noted, not yet acted on). Verified live in the Browser pane: DETAIL section appeared automatically, clicking "Fuel Petcocks" correctly filtered 941→190 parts. Reindexed clean (90,483 docs, 0 errors).

**Reusable going forward**: any oversized subcategory (Laken's own examples: Gas Tanks & Gas Caps solved; Engine's genuinely-generic-hardware 29-item remainder from session 86 is too small to bother, but things like Front Fender & Hardware at 523 or Oil & Fuel Lines at 737 are candidates) can get the same tier-3 treatment by adding one more classifier function to `rebuild_subcategory_detail.mjs` — audit sample → dry-run coverage → write-shadow → promote → reindex. No new frontend/backend code needed, ever, for this — it was all built once, previously, and just needs data.

### Pass 10 — "Mounts, Housing & Hardware" (477) was hiding five entirely unrelated product families, none of it ever inspected
Laken spotted "Amp Socket Wiring Housing" (2/3/4/6/8-position electrical connectors) live on the PDP admin panel, correctly called it Electrical, not Dashes & Gauges. Investigating turned up the actual root cause: this bucket was built in **part 2** of this session as a blind rename of the pre-existing `Housings`/`Gauge Hardware`/`Instrument Hardware` subcategories (session 80 origin) — the one part of today's work that got a rename without a content audit, unlike every other bucket touched today. Laken's response, correctly: **"the entire housing subcat needs to be audited"** rather than just fixing the one flagged cluster — a full read of all 477 rows found the 6-item wiring-connector cluster was one of five distinct misfiled product families:
- **78 → Handlebar & Controls / Bar Ends, Throttle Tubes, Throttle Assists, Hand Control Hardware**: handlebar switch housing kits (turn signal/horn/high-beam, ~50 SKUs across chrome/black/matte/left/right/upper/lower/cruise/radio variants), twist-grip housings, throttle housings, knurled handgrips.
- **34 → Electrical / Sensors & Switches**: oil pressure/temp sending units, EFI throttle position sensors, speed sensors (incl. throttle-by-wire twist-grip sensors — sensor-ness took priority over housing-type in the classification order), ignition sensor screws.
- **24 → Dashes & Gauges / Misc Gauges**: real gauge/meter *instrument products* that had been mislabeled as hardware purely because their names contain the word "Housing" describing their own case (multifunction meters, hour meters, Fairing-mounted PSI gauges, LCD color-change speedo/tach displays, a TNT-01 combo unit) — caught on a second pass after noticing the first regex missed these because they don't contain mount/bracket/adapter-type words.
- **19 → Transmission & Clutch / Shift Linkages & Levers**: novelty/motor-era shifter knobs (Knucklehead, Panhead, Shovelhead, Evolution, skull/dice/police-badge styles) — completely unrelated to gauges, just happened to land in the old "Hardware" bucket.
- **7 → Electrical / Wiring & Components**: the originally-flagged Amp Socket/Mate-N-Lock connector housings + wire conduit.
- **4 → Dashes & Gauges / Speedometers**: "Handle Bar Mount Speedometer" units — these are whole speedometers (handlebar-mount style), not mounting hardware for one.
- **3 → Transmission & Clutch / Primary & Derby Covers**: primary housing bearing/seal, shifter shaft housing seal.
- **2 → Tanks & Body / Oil Tank & Dipsticks**: 2 more dipstick items using "Housing" instead of "dipstick gauge" phrasing — same false-positive pattern as the 8 caught earlier this session, just missed because the earlier search was keyed on "dipstick"/"oil plug" exact phrases.
- **1 each → Electrical / Audio & Communication** (speaker bezel), **Electrical / Starter Motors, Solenoids & Accessories** (starter housing thru-bolts, "Stater" typo in source name), **Electrical / Charging System & Components** (generator bearing housing).

**174 of 477 rows (36%) redistributed**, leaving 303 in Mounts, Housing & Hardware. **5 rows deliberately left untouched, not force-classified**: 3 "Bearing Housing Right/Right Side" (no system named in the product name — could be primary, transmission, or wheel) and 2 "Pin-Lock"/"Security & Key Knob" items (likely seat or luggage quick-detach hardware, but not confirmed) — same "don't force a fallback bucket" principle used throughout this project. Applied via `category_cleanup_20260715_part10.sql`, same backup + BEGIN/COMMIT convention; all destination counts verified exact against the pre-apply proposal, 90,483 active rows unchanged. Reindexed.

**Standing lesson, sharpened**: a rename between two subcategory *names* is not automatically content-safe just because it doesn't involve keyword classification — this pass proves a "clean rename" can still be silently wrong if the source bucket's contents were never actually sampled. Every bucket in a taxonomy pass needs at least a spot-check read of real rows before being trusted, even ones that look like pure renames.

### Pass 11 — same session, pass 10's own regex still wasn't clean
Laken caught it browsing the live site: multiple whole speedometer/tachometer *products* were still sitting in "Mounts, Housing & Hardware" after pass 10 (e.g. "2.4" MPH Mini LED Mechanical Speedometer/Indicators", "OE Style Tachometer - Stainless Steel Bezel"), plus a handlebar clamp. Root cause: pass 10's regex correctly caught mounting-hardware *patterns* (mount/bracket/adapter/clamp) but missed whole instruments that use "Housing"/"Bezel"/"Face"/"Diameter" as descriptors of the *instrument itself* rather than separate mounting hardware for one — e.g. "52mm Chrome Speedometer Housing" is a complete speedometer sized 52mm, not a housing accessory for one. Also missed: 3 more "switch housing" items with two stacked descriptor words ("Control Plain Switch Housing", "Left Side Upper Switch Housing" — pass 10's regex only handled one optional descriptor, not two), 3 abbreviated "HOUSING THR..." throttle-housing items, 2 "Indicator Light Handlebar Clamp" items, and 2 "Instrument Harness Assembly" wiring items.

This time used **hand-verified explicit ID lists instead of another regex pass** — 51 items total, all cross-checked one by one: 14 → Speedometers, 8 → Tachometer, 19 → Misc Gauges (combo speedo/tach housings, KOSO/multifunction-style gauges, gear indicators, pressure-gauge-and-bracket kits), 8 → Handlebar & Controls, 2 → Electrical/Wiring & Components. Applied via `category_cleanup_20260715_part11.sql`. Ran a final sanity sweep afterward for every tell-tale "this is a whole instrument, not hardware" signal (RPM/MPH/KPH, gear ratios like `2:1`, "Face", "Display", or `speedometer|tachometer` combined with `bezel`) plus the switch-housing/throttle/harness patterns — **zero remaining matches**, confirmed clean. 90,483 active rows unchanged throughout. Reindexed.

**Final Dashes & Gauges, both passes combined**: Speedometers 387 / Tank Dashes/ Console & Accessories 271 / Mounts, Housing & Hardware 251 / Misc Gauges 202 / Tachometer 47.

**Meta-lesson**: this project's regex-based bulk classification is reliably good at catching what it's looking for and reliably bad at catching what it *isn't* looking for — a keyword list built from one reading pass will miss product-naming variants (word-order changes, abbreviations, a-whole-product-described-by-an-accessory-word) that only show up on a second or third read. When precision matters more than speed on a smaller remaining set (tens of rows, not hundreds), hand-verified explicit ID lists found the last mile faster and more reliably than iterating the regex again.

---

## Same session — three more finalized specs: Frames & Suspension, Hardware, Accessories & Gear ("audit heavily and thoroughly")

Laken handed over three more finalized target specs at once, explicitly asking for heavy, thorough auditing — a direct response to today's earlier lesson that blind renames hide contamination. Applied that standard throughout: every subcategory sampled for real content before mapping, not just renamed. All three finished, row integrity held at 90,483 throughout every pass.

### Hardware — 3,288 rows, the big one: a full hardware "dumping ground" pattern, same as session 86's Engine Parts
Laken's spec: Bolts & Screws / Nuts & Washers / Clips & Clamps / Spacers / Fittings / Pins / Bolt Caps / Heat Shrink, Zip Ties, Etc. — plus an open question, **"Drain Plugs move to oil tank category??"**

First finding: the 3,163-row "Bolt Kits, Hardware Assortments & Replenishment" mega-bucket wasn't just unsplit generic hardware — ~400 rows were **system-specific hardware kits that belonged in their owning category entirely**, not Hardware (dash cover kits, primary/transmission mounting kits, engine fastener kits, brake caliper/rotor mounting kits, docking hardware, gas tank mount kits, etc.) — the exact same "big hardware dumping ground" pattern found in session 86's Engine Parts pass and this session's Dashes & Gauges "Housing" bucket. Flagged this to Laken before proceeding rather than silently expanding scope; confirmed **"Full redistribution now."**

Built `redistribute_hardware_misc_20260715.mjs` (explicit ID lists, hand-classified from a full read of all 398 candidate rows) + two SQL follow-up passes for duplicate-named items the ID lists missed. **295 + 91 + a handful more redistributed across 15 different categories**: Engine (Engine Accessories, 63), Transmission & Clutch (Transmission Parts, 133 incl. all the Inner Primary/External Primary mounting-kit variants), Dashes & Gauges (Tank Dashes/Console, 21), Tanks & Body (25, gas tank mount kits + fender mount kits + a stray license-plate-fastener pack), Frames & Suspension (Trike Kits & Sidecar Parts + General + Frames, chassis fastener kits), Foot Controls (Kickstands + Footpegs, 9), Handlebar & Controls (7), Windshields & Fairings (4, "Switchblade" is a Klock Werks windshield line, not a knife), Electrical (6 subcats, 30 total — wiring, points/distributors, sensors, starters, audio, batteries), Lighting (5), Brakes (Hardware, 24 — caliper/rotor mounting kits), Exhaust (6, pipe brackets), Seating (Seat Hardware, 8), Luggage & Racks (General Accessories + Docking, 22), Accessories & Gear (Towing Equipment + Merchandising, 8), Wheels & Tires (Axles & Spacers, 3), Carburetion & Fuel (Air Cleaners + Carburetors, 12), Tools & Chemicals (1).

**Answered Laken's drain-plug question — it wasn't a clean yes**: "Timing Drain Plugs" (53) turned out to be two unrelated product types sharing one bucket name. 22 were **crankcase timing-inspection plugs** (used to view timing marks — nothing to do with draining anything) → **Engine**. Of the genuine drain plugs: 9 explicitly named "oil tank" → **Tanks & Body/Oil Tank & Dipsticks** (yes, this part of the question), 5 transmission/primary-specific → **Transmission & Clutch**, 3 engine-case-specific → **Engine**, and 14 generic ones with no system named defaulted to **Tanks & Body** (oil tank is by far the most common real-world referent for a bare "drain plug," confirmed by the ratio of explicitly-named ones).

Final generic 8-way split (2,890 rows after redistribution): **Bolts & Screws 1,234 / Nuts & Washers 744 / Bolt Caps 423 / Clips & Clamps 131 / Spacers 120 / Heat Shrink Zip Ties Etc. 62 / Pins 39 / Fittings 37**. The pre-existing "Bolt Caps, Plugs, Shrink Tubes, Cable Ties, Wire Wraps" (72) and Timing Drain Plugs (53) buckets were folded into this same pass. **47 rows left flagged, not force-classified** (ball bearings, generic rubber bushings, bare "HARDWARE KIT" with zero descriptor, "THUNDER TORQUE INSERT" torque-limiting fasteners) — pushed into the new manual-review-queue bucket (see below) instead of a prose note this time.

**Bug reconfirmed twice more this pass**: wrote `\bcap\b`, `\btool\b`, `\bnut\b`-adjacent patterns in Postgres regex again (Postgres `~*` doesn't support `\b`) — caught both times by a 0-match sanity check before trusting the tally, not after applying. Also found a **comma-boundary gap**: `(\s|$)` doesn't match "HEX NUTS, 1/4 inch..." where the boundary character is a comma, not whitespace — fixed to `(^|[\s,])...(\s|[\s,]|$)` pattern going forward.

### Frames & Suspension — 4,089 rows, clean-looking buckets still had real contamination
Laken's spec: Front Forks / Triple Trees / Shocks / Frames / Trike Kits & Sidecar Parts / General.

- **"Forks" (1,930)**: hid a 24-item Spring-Fork headlamp cluster (→ **Lighting/Headlights** — "Spring Fork" here describes the fork-mounted headlight style, not the suspension part), 3 "Andrews/45 W/G Shift Fork Assembly" items (a transmission part, not suspension — → **Transmission & Clutch**), and 18 fork-service tools (→ **Tools & Chemicals**). Renamed remainder to Front Forks (1,886).
- **"Frame" (683)**: hid 76 sidecar-specific parts (tie rods, axles) → **Trike Kits & Sidecar Parts** (joining the existing Trike Conversion Kits, now 107 total) and 1 swingarm-bearing-puller tool → Tools & Chemicals. Renamed remainder to Frames (606).
- **"Triple Trees & Covers" (429)**: essentially clean — the "non-triple-tree" scan flagged nothing real except vocabulary my first pass missed (Top Tree, Stem Nut, Steering Bearing, Top Clamp are all triple-tree/steering-stem terms) and one true fork-assembly misfit. Renamed to Triple Trees (428).
- **"General Accessories" (88)**: a genuine mixed bag. ~37 Highway/Freeway Bar items **duplicated Foot Controls' own existing "Highway Bars & Pegs" bucket** — moved there (matches the established session-84/85 precedent that crash/highway bars belong in Foot Controls). 9 Trailer Hitches → **Accessories & Gear/Towing Equipment** (today's other new spec). 1 "Indian Front Chrome Shock" was literally a shock sitting in the wrong bucket → Shocks. 1 "Springer Fender Mounts" → Tanks & Body. Remaining 42 (skid plates, chin spoilers, stabilizer links, tie-rod ball joints) → General.
- Final: **Front Forks 1,886 / Shocks 924 / Frames 609 / Triple Trees 428 / Trike Kits & Sidecar Parts 107 / General 42.**

### Accessories & Gear — 1,844 rows, clean split once the real contents were sampled
Laken's spec: Swag/Merchandising, Phone Accessories, Shop Books, Motorcycle Covers, Handlebar Accessories, Towing Equipment (Locks, Trailer, Straps, Chocks).

Merchandising (628, patches/shirts/keychains/pins), Motorcycle Covers (138), and Shop Manuals (→ Shop Books, 186) were all clean on inspection — straight renames. "Electronics & Mounts" (576) is almost entirely phone cases, screen protectors, and RAM-style ball/clamp mounts → **Phone Accessories**. "Drink Holders & Coolers, Flags, Flagpoles & Accessories" (81) → **Handlebar Accessories** (drink holders, flag mounts, cup holders — all handlebar-clamped), minus 3 fuel petcocks that only landed here because their finish name says "Flag Chrome" (not actually flag-related) → Tanks & Body/Oil & Fuel Lines. "Security" (105, chain/disc/pad locks, alarm disc locks, ground anchors) merged straight into **Towing Equipment** — matches Laken's own bucket description, which explicitly lists "Locks" as part of Towing Equipment, resolving what would've otherwise been a category with no home in the new 6-bucket spec.

Final: **Merchandising 628 / Phone Accessories 576 / Towing Equipment 235 / Shop Books 186 / Motorcycle Covers 138 / Handlebar Accessories 78.**

### New tool used for the first time this pass: the manual-review-queue bucket
The 47 flagged Hardware leftovers are the first bulk use of `/admin/review-queue` (built earlier this session) instead of a prose "held back" note in this log — confirms the bucket works for exactly the use case it was built for.

### Cleanup: one naming inconsistency caught on final live-site verification
The Hardware redistribution script created a new `Brakes / Hardware` subcategory (24 rows) instead of merging into the pre-existing `Brakes / Brake Hardware` (695 rows) — same content, two different label strings. Caught by pulling the live `/categories` breakdown after the session's reindex and eyeballing every category, not by a dedicated audit. Merged, reindexed. **Verified final state live**: 90,483 active products, 22 categories, 173 subcategories, 0 uncategorized, Typesense collection count matches Postgres exactly throughout every reindex this session.

### Follow-up, same session: tier-3 detail classifiers for every bucket over 150 rows, across all 6 categories audited today
Laken: "can we run the details tier category option on any buckets over 150 in regards to all the sections we have audited." 25 subcategory buckets qualified (>150 active rows) across Windshields & Fairings, Dashes & Gauges, Tanks & Body, Frames & Suspension, Hardware, and Accessories & Gear — Gas Tanks & Gas Caps already had this from earlier the same session (1,389/1,397, 99%). Sampled real content for the other 24 (not guessed from the subcategory label) and added one classifier each to `rebuild_subcategory_detail.mjs`, e.g.: Windshields split by tint (Clear/Smoke/Dark Smoke/Solar/Light Tint), Speedometers by type (GPS/Electronic/Mechanical/Faces/Housings/Speedo-Tach Combos), Front Forks by part type (Tubes/Sliders & Assemblies/Internals/Springer/Steering Dampers/Hardware), Bolts & Screws/Nuts & Washers/Bolt Caps each split by fastener subtype, Merchandising by product type (Apparel/Patches/Belt Buckles/Décor & Models), Towing Equipment by function (Locks & Alarms/Chains/Straps & Tie-Downs/Cargo Nets/Trailer Hitches/Anchors).

**Coverage after one refinement round**: mostly 60–99%, two buckets (Windshields & Fairings/Fairings, Tanks & Body/General) got a bare-keyword fallback rule added after the first dry-run showed real diversity the initial rules missed (fairing-brand-specific naming, ornament/chap/brace fender accessories) — landed at acceptable coverage per this script's stopping rule rather than chasing every last row. 62 total classifiers now live in the script (38 from prior sessions + Gas Tanks & Gas Caps + these 24), ~32,000 rows eligible catalog-wide. Applied via `--write-shadow --promote`, reindexed. Verified live in the Browser pane against a real product page.

**Note for a future session**: several of the pre-existing classifiers (built in earlier sessions, before today's category renames) are now permanently dead — keyed to category|subcategory combos that no longer exist (e.g. `'Suspension|Fork Tubes & Internals'`, `'Fenders & Body|Windshields'`, `'Frame & Hardware|Hardware & Fasteners'`). Their old output values persist on rows via the shadow column (never cleared), so a `has_detail` count > 0 on a bucket doesn't necessarily mean a *live* classifier covers it — worth a cleanup pass to prune dead keys and confirm no bucket is silently relying on stale detail values computed under a retired category boundary.

### Seating — 3,717 rows, a genuinely different kind of split: model family, not part type
Laken's 12-bucket spec (Solo Seats, Pillion Pads, Softail (FL/FX)(FLS/FXS), Dyna (FXD), Touring (FLH/FLT), Sportster (XL), Chopper/Rigid, FXR, Touring Back Rest, Seat Knobs/Screws, Seating Hardware, Accessories) splits primarily by **which motorcycle platform the seat fits**, not by product type — a first for this session. Model codes turned out to be reliably present in most product names (FLST/FXST = Softail, FXD = Dyna, FLH/FLT = Touring, XL = Sportster, FXR = FXR), so name-based priority-ordered classification worked without needing to join fitment tables.

**Two real bugs found applying the classifier**: (1) the project's recurring Postgres-`\b` bug struck a third+ time this session — `\bdyna\b` and `\bxl\b` silently matched nothing, which is why the first dry-run showed Sportster at only 14 rows; fixed with the standard `(^|[\s-])...([\s-]|$)` pattern and Sportster jumped to 234. (2) Softail model codes needed real breadth — `FXSB`/`FXBR`/`FXBB`/`FXFB`/bare `FLS`/`FXS` (no third letter) all needed adding beyond the obvious `FLST`/`FXST`/"Fat Boy"/"Softail".

**213 saddlebag items were misfiled into Seating entirely** (172 in "Seat Hardware", 41 in "Seats" itself) — not seat parts at all, just happened to share a "Seat Hardware" or generic "Seats" bucket with genuine seating parts. Moved to Luggage & Racks/Saddlebags (421 existing, exact name match, now 634 total).

Final: **Softail (FL/FX)(FLS/FXS) 1,055 / Touring (FLH/FLT) 678 / Seating Hardware 429 / Touring Back Rest 248 (clean rename, content confirmed overwhelmingly touring/sissy-bar-pad context) / Dyna (FXD) 239 / Solo Seats 236 / Sportster (XL) 233 / Accessories 54 / Seat Knobs/Screws 48 / Pillion Pads 37 (drawn from both the old "Seats" bucket's pillion-named rows and "Seat Pads & Covers"' actual pillion-pad products) / FXR 28 / Chopper/Rigid 11**. 208 rows (7.6%) left in a generic "Seats" bucket — no model code parseable from the name (e.g. "Corbin Gentry Banana Seat," "Wide Tripper™ Seat" with no listed fitment) — accepted as residual per this session's established stopping-rule convention rather than force-guessed.

**Immediate follow-up, same session**: Laken caught that "Touring Back Rest" (and a few rows scattered into "Seats" and "Seating Hardware") still contained 132 **sissy bar pad** products — correctly identified as belonging with Luggage & Racks' existing "Sissy Bars" bucket (567 rows), not Seating at all. Moved all 132 (104 from Touring Back Rest, 24 from Seats, 4 from Seating Hardware) → Luggage & Racks/Sissy Bars, now 699. Touring Back Rest down to 144 (pure driver/passenger backrests and Tour-Pak pads, no sissy-bar-pad overlap left). Reindexed.


**Final Dashes & Gauges after passes 8–9**: Mounts, Housing & Hardware 477 / Speedometers 369 / Tank Dashes/ Console & Accessories 270 / Misc Gauges 159 / Tachometer 39.

### Still open — carried forward from session 86, unchanged
1. **Chopper Supplies** — still at 0 rows, no subcategory scheme ever built. Lowest priority.
2. **`merge_catalog_unified.js` rebuild risk still unresolved** — none of today's or session 86's manual SQL cleanup is ported into `mapDisplayCategory()`; a future full `catalog_unified` rebuild would still silently undo all of it. Explicitly discussed this session and deliberately **not** fixed yet — decided a rules-as-code port would need rewriting every session while categories are still actively being reorganized (today is a perfect example: 3 more categories just changed shape). Recommended approach when the taxonomy actually stabilizes: a snapshot/reconciliation table (id → display_category/display_subcategory) reapplied post-rebuild, not hardcoded classification rules.
3. **`app/admin/catalog/CatalogTable.jsx`'s own stale category array** — left alone, see above.
4. **Laken's "full and final audit" is an ongoing multi-category handoff** — 3 of however-many categories done this session (Windshields & Fairings, Dashes & Gauges, Tanks & Body). Expect more category specs to arrive the same way (exact target subcat names given up front) in future sessions; the audit → sample → propose-with-counts → apply → verify → reindex loop used today is the pattern to repeat.

---

## Session 86 (July 15 2026) — Interactive SQL cleanup pass: Accessories & Misc + Security & Covers fully dissolved, Hardware Covers & General split in two, Engine's two mega-buckets (Engine Parts 733→29, Engine Accessories 1041→2) broken apart, Windshields (1242→933) split into real subcategories

**Different working style this session** — no `.mjs` audit/dry-run/apply scripts. Laken worked directly in chat with a psql connection open, reviewing real product-name samples live and calling merge/split targets in real time. All changes went straight through `scripts/ingest/category_cleanup_20260714_part1.sql` through `_part14.sql` (filenames carry the 07-14 date the session started on; the session itself ran into 07-15), each wrapped in its own `BEGIN`/`COMMIT` with a matching `category_cleanup_backup_20260714[_partN]` snapshot table created first — full row-level rollback available for every single change this session, not just the risky ones.

### Pass 1 — first wave of merges + two category dissolutions
Straightforward small/duplicate-subcategory merges across Engine (Bearings & Bushings/Connecting Rods → Engine Parts/Bottom End), Transmission & Clutch (Hydraulic Clutch Kits/Chains & Hardware/blank rows), Electrical (Ignition Coil Hardware/Generators & Starters/Starter Relays/Batteries/Connectors & Terminals/LED Bulbs), Handlebar & Controls (Grips/Throttle Assembly/Switches & Controls), Foot Controls (Kickstands/Freeway Bars/Forward & Mid Controls), Lighting (Underglow & Neon), Seating (Pads & Pans), Cables (Idle/Throttle/Speedometer/Clutch/Clamps and Covers), Tanks & Body (Fender Trim/Gas Tanks, plus moving Fuel Valves & Filters (Carb) → Carburetion & Fuel), Tools & Chemicals (Cleaners & Detailing → Chemicals & Lubricants). **Security & Covers dissolved** (Bike Covers + Shelters & Storage → Hardware Covers & General/Motorcycle Covers; Security → Accessories & Misc, temporarily — see pass 3). While in Accessories & Misc, also found and fixed items that had nothing to do with "misc": Cooling Systems (13) → Engine/Engine Accessories, bulk cable/hose stock (6) → Cables/Universal Build Your Own, Decals & Emblems (14, 3 of which were a misfiled horn/coil-cover/baseball-hat) → Hardware Covers & General/Merchandising.

### Pass 2 — Frames & Suspension + Luggage & Racks follow-ups
Springer Fork (21, under the 25-item floor) merged into Forks. Transmission & Clutch's Mechanical Reverse Kits (22) merged into Gear Sets. Tanks & Body's 46 NULL-subcategory rows (side covers, tail sections, fairing/windshield parts, a dash panel, running boards, a turn-signal grille, Boom Mat) sorted individually into Transmission & Clutch/Dashes & Gauges/Windshields & Fairings/Foot Controls/Lighting/Tanks & Body's own Fender Parts & Accessories.

### Pass 3 — Accessories & Misc dissolved entirely
921 active items across 5 subcategories routed out: Electronics & Mounts (565, phone/GPS mounts+cases) and Books & Manuals (135, merged into existing Shop Manuals) and Security (105) and Trailer & Towing+Tie-Downs & Transport (113 combined) all → Hardware, Covers & General. 3 items hiding in Tie-Downs & Transport ("Black/Chrome Ignition Coil Strap") were actually coil-mounting hardware, not tie-downs → Electrical/Ignition Coils. Category confirmed at 0 active rows after.

### Pass 4 — Luggage & Racks' two catch-alls broken into real bag-type subcategories
"Bags & Packs" (166) + "Luggage Parts" (30) dissolved into: Tank Bags (12), Handlebar Bags (36), Tail Bags (18), Tool Bags & Pouches (24, incl. 3 fork bags), 12 windshield/fairing storage pouches moved OUT to a new **Windshields & Fairings/Windshield & Fairing Bags** subcategory, 4 Tour Pak accessories merged into the existing Tour Pak subcat, 3 "Tank Pad" items (not bags — tank protector pads) → Tanks & Body/Fender Parts & Accessories, 1 phone mount → Hardware Covers & General/Electronics & Mounts. The 75 true leftovers (backpacks, duffels, dry bags, rack bags, coolers, pet carriers, holsters, stash tubes, registration holders) relabeled **General Accessories** — matching naming already used in Frames & Suspension rather than leaving two separate vague buckets.

### Pass 5 — a misfiled Transmission & Clutch subcategory found by inspection, not audit
Laken flagged "there is an oil system in transmissions as well" after the Engine Oil Pumps + Breathers & Oil System merge (pass 1) — turned out Transmission & Clutch's own 26-row "Oil System" subcategory was entirely engine oil-pump internals (feed/scavenger idler gears, oil control valves, filter brackets — textbook engine oil-pump component names, nothing primary/chaincase-specific). Moved wholesale into Engine/Oil Pumps & System, now 851 items.

### Pass 6 — Hardware, Covers & General split into two top-level categories
At 3,296 Bolt Kits + 1,798 unrelated accessory items under one "general" name, Laken asked for a recommendation and then confirmed the split: **Hardware** (Bolt Kits/Hardware Assortments & Replenishment 3,087, Bolt Caps/Plugs/Shrink Tubes/Cable Ties/Wire Wraps 72, Timing Drain Plugs 53 — 3,212 total) vs. **Accessories & Gear** (Merchandising 608, Electronics & Mounts 566, Shop Manuals 186, Motorcycle Covers 138, Trailer & Towing 113, Security 105, Drink Holders & Coolers/Flags 82 — 1,798 total). Category count went from 21 to 22 (2 dissolved categories net minus 1, plus this split net plus 1).

### Pass 7 — Transmission & Clutch's "Clutch Cables & Components" (75) was three different product types
Laken's instruction was "bring cables to the top level cables category," but full inspection showed the 75-item subcategory was actually: 40 genuine clutch lines/hoses (→ Cables/Hydraulic Clutch Lines), 9 foot-clutch pedal hardware (old-school foot-operated clutch mechanism, corrected by Laken from an initial wrong guess of "Handlebar & Controls" — not a hand control at all → Foot Controls/Brake Arm & Pedal Hardware), and 26 handlebar-mounted clutch lever/perch hardware (→ Handlebar & Controls/Hand Control Sets, Levers, confirmed by Laken: "the perch parts need to go to handlebars"). Subcategory emptied.

### Pass 8-11 — Engine > Engine Parts: 733 → 29 items
Laken's original flag was "a ton of Deutsch electrical parts in there that should be in electrical" (26 wiring-connector items) — but a full read of all 733 names showed the whole subcategory was a dumping ground, not just the Deutsch items ("pretty much all the items within this subcategory need to be correctly routed"). Four SQL passes routed out, in order of volume: the entire **Ultima starter motor family** (~80 items — motors, brush plates, armatures, yoke assemblies, housings) + generators/alternators + ignition system (distributors, points, magnetos, coil sets, timer/ignition modules) + wiring connectors/terminals + sensors/switches/horn mount → **Electrical**; carb rebuild kits + EFI tuners (ThunderMax/Twin Tec/Dynatek) + intake/fuel-system parts → **Carburetion & Fuel**; kickstart hardware + primary/derby cover parts + transmission dipsticks/sprockets/shift linkages + 4-speed lock tab washers → **Transmission & Clutch**; actual Knucklehead replica frames + misfiled fork-plug O-rings (forks are suspension, not engine) + swing arm bolts → **Frames & Suspension**; seat springs/solo seat brackets/seat post hardware → **Seating**; wheel hub rebuild kits → **Wheels & Tires**; brake lever/caliper/drum parts → **Brakes**; exhaust hanger brackets/muffler hardware → **Exhaust**; headlight/taillight/visor items → **Lighting**; oil-temp gauges/dash retainers/speedometer drive units → **Dashes & Gauges**; generic dimension-only bolts with no engine-specific fitment → **Hardware**; Spectro/Oilzum oil products + brake fluid (lubricant products, not parts) → **Tools & Chemicals**; novelty merchandise (engine patches, wall plaques, scale model motors) + a phone/camera mount cluster → **Accessories & Gear**. Final 29-item remainder is genuinely ambiguous generic crankcase hardware (case sets, retainer rings/clips, pivot plates) with no clean subcategory home — accepted as the legitimate catch-all remainder.

### Pass 12 — Engine > Engine Accessories: 1,041 → 2 items, but a different kind of problem than Engine Parts
Full inspection showed this bucket was NOT full of miscategorized junk like Engine Parts — nearly every item genuinely was an engine cover (cam covers, derby covers, points covers, pushrod covers, rocker box covers, timing covers, inspection covers). The real problem was one undifferentiated 1,041-item bucket instead of real, browsable subcategories. **Laken corrected two of the proposed splits before applying**: "Timing Cover and Timing Points Cover are the same thing... i believe the covers are located in points distributors and accessories" (133 items → **Electrical/Points, Distributors & Accessories**, not a separate Engine "Timing Covers" bucket) and "derby cover is in transmission already" (139 items → **Transmission & Clutch/Primary & Derby Covers**, along with 6 "Outer Primary Cover" items that used different wording for the same product family). Remaining split, all staying in Engine: **Cam Covers** (236), **Rocker Box Covers** (176, absorbed lifter/tappet/valve covers too), **Pushrod Covers** (163), **Engine Dress-Up Kits** (62, head-bolt/spark-plug decorative covers), **Inspection Covers** (49), **Cooling System** (18). Also routed out: ignition coil covers/sensor covers/generator covers/starter-gear covers → their respective Electrical subcategories, a carb air-cleaner cover → Carburetion & Fuel, and 9 novelty items (belt buckles/coat hooks/towel rings shaped like cam covers, not functional engine parts) → Accessories & Gear/Merchandising.

### Pass 13 — small follow-up merge, Laken's call
Luggage & Racks' Tail Bags (18) + Tank Bags (12), both newly created in pass 4, folded straight into General Accessories per Laken ("throw tail bags and tank bags together into general accessories") — now 105 items.

### Pass 14 — Windshields & Fairings > Windshields: 1,242 → 933, same undifferentiated-bucket pattern as Engine Accessories
Split into **Windshield Hardware** (214, clamps/brackets/mounts/bushings), **Windshield Trim** (59), **Deflector Screens** (19, a distinct smaller product from full windshields). 10 misfiled actual fairing panels (ANARCHY SEMI FAIRING, CARBON FIBER FAIRING LOWERS, WAVE QUICK RELEASE FAIRING, etc.) merged into the existing Fairings/Lower Fairings subcategories. 6 true outliers had nothing to do with windshields at all: an engine heat shield → Engine/Engine Accessories, a taillight assembly → Lighting/Taillights, a headlight lens + correction ring → Lighting/Headlights, a turn-signal relocation kit → Lighting/Turn Signals.

### Net result this session
**display_category: 23 → 22** (Accessories & Misc and Security & Covers both dissolved to 0 rows; Hardware, Covers & General split into Hardware + Accessories & Gear, net -1). **Subcategory count: up, not down, this session** — unlike prior sessions' consolidation work, this session mostly *created* new, more granular subcategories out of oversized undifferentiated buckets (Engine's two mega-categories, Windshields, Luggage's two catch-alls) rather than merging existing ones down. Verified live: **90,483 active products, 22 categories, 167 subcategories**, zero NULL `display_category`/`display_subcategory` anywhere in the catalog. **9 SKUs soft-deactivated** this session (2 Gasket Board sheet-stock items, 7 "REVO ARC Canister Decal" color variants under Dashes & Gauges) — `is_active=false`, not deleted, per Laken's explicit confirmation after an initial ambiguous answer was caught and re-asked.

### Still open — not urgent, no deadline attached
1. **Chopper Supplies** — still at 0 rows, no subcategory scheme ever built. Unchanged for many sessions running, lowest priority.
2. **This session's `category_cleanup_20260714_part*.sql` files are NOT wired into `merge_catalog_unified.js`** — unlike the durable fixes from sessions 74-81 (brand normalization, display-fixture exclusion), these were one-off interactive `UPDATE` passes against already-live data, not classification rules baked into the ingest pipeline. **A future full `catalog_unified` rebuild (TRUNCATE + reinsert from `pu_catalog`/`wps_catalog`/`vtwin_catalog`) will silently undo every merge/split from this session** unless the same category/subcategory logic gets ported into the merge script or a post-rebuild reconciliation pass. Flag this before the next full rebuild, not after.
3. **Frontend hardcoded category array** — same standing issue noted every session since 77, now also stale on `Hardware, Covers & General` no longer existing at all (split into `Hardware`/`Accessories & Gear`) and `Security & Covers`/`Accessories & Misc` both gone. Still not fixed.

---

## Session 85 (July 14 2026) — Three duplicate/overlapping categories consolidated: Fenders & Body, Suspension, and Frame & Hardware all retired

**Follow-up to session 84's reopened Suspension-vs-Frames & Suspension flag.** Full pattern: audit (subcategory breakdown + duplicate-name check against the merge target) → dry-run → Laken's calls on ambiguous rows → `--apply` → sync/reindex. All three merges applied clean, no errors.

### Fenders & Body — MERGED into Tanks & Body, retired at 0 rows
- Final 26 active rows (Gas Caps & Petcocks 14, Gas Tanks 12) moved into Tanks & Body/Gas Tanks & Gas Caps via `fix_fenders_body_merge.mjs --apply` — plain category-only move, no dedup.
- Audit (`audit_fenders_body_merge.mjs`) found 16 of 26 rows had name-similarity matches against existing Tanks & Body rows, but these were cross-vendor near-duplicates (same style of product, different vendor), not true row-level duplicates — not a blocker.
- 2 internal PU near-duplicate pairs found *within* Fenders & Body itself (`[788]`/`[30091]` both "Live to Ride Console Door - Chrome with Gold"; `[30228]`/`[30163]` both "...Chrome") — Laken's call: leave as-is for now, not resolved.

### Suspension — MERGED into Frames & Suspension (+ 2 other categories), retired at 0 rows
- All 454 rows merged via `fix_suspension_frames_merge.mjs --apply`. Mapping: Shocks & Springs (230) → Rear Shocks & Lowering Kits; Fork Tubes & Internals (98) → Forks; Triple Trees & Stems (44) + Steering Stem Hardware (5) → Triple Trees & Covers; Swingarms (16) → Frame; Ride Control & Rear Support (9) → Rear Shocks & Lowering Kits; Lowering & Lift Kits split — 18 "TRIKE CONVERSN" ids + 2 trike comfort lift kits → Trike Conversion Kits, remaining 13 → Rear Shocks & Lowering Kits; Fork Lowers & Sliders split — 5 fork-leg rows → Forks, 1 stray Turn Signal Eliminator → **Lighting**/Turn Signals (not suspension at all); Dampers & Cush Drive split — 7 steering-damper rows → Forks, 6 cush-drive rows → **Transmission & Clutch**/Transmission Parts (drivetrain part, not suspension — Laken's call).
- Duplicate-check audit confirmed the "duplicate" name matches against Frames & Suspension were cross-vendor product-family overlap (different sizes/colors/specs of the same style of shock/triple-tree), not true row-level duplicates — no dedup needed, this was purely a subcategory-mapping job.
- 7 rows left `display_subcategory = NULL` after the main merge (the 6 cush-drive + 1 turn-signal rows moved to other categories) were resolved in a follow-up (`fix_suspension_merge_final_subcats.mjs --apply`) once Laken picked real subcat names off a live query of each destination category's existing subcats.

### Frame & Hardware — CONSOLIDATED across 4 destinations, retired at 0 active rows
- Turned out to be a duplicate-hardware-category problem, not a clean 1:1 merge target: only its "Hardware & Fasteners" subcategory (1,896 rows) was a genuine duplicate of Hardware, Covers & General's "Bolt Kits, Hardware Assortments & Replenishment" (confirmed by dozens of near-identical bolt-kit/nut/washer name matches in the audit). The other 4 subcats were frame/body parts miscategorized at the top level, not hardware.
- Full mapping via `fix_frame_hardware_consolidate.mjs --apply`: Hardware & Fasteners (1,896) → Hardware, Covers & General/Bolt Kits...; Frame Parts (166) → Frames & Suspension/Frame; Body Panels (46) → Tanks & Body; Protection (40, engine guards/crash bars) → Foot Controls/Highway Bars & Pegs (same crash-bar-as-highway-bar pattern from session 84); Kickstands (4) → Foot Controls/Kickstands.
- Final 2 "FILLER HOSE" rows (`[57338]`, `[57337]`, WPS) — previously held back across two sessions — deactivated (`is_active = false`) per Laken's explicit call to remove them, using the project's standing convention (hard DELETE fails, `catalog_unified.id` is FK-referenced by `product_vendors`).

### Still open — not urgent, no deadline attached
1. **Chopper Supplies** — still at 0 rows, no subcategory scheme ever built. Scope narrowed (session 81) to bulk/raw-material consumables only; `audit_chopper_supplies_scope_v2.mjs` drafted but never run. Lowest priority, unchanged for several sessions running.
