...
**Last Updated: July 12, 2026 (Session 81)**
...
## ▶ NEXT UP — Accessories & Misc reclassification (drafted, not applied), Chopper Supplies v2 audit, then remaining health-check targets
...
**Frames & Suspension is done** (session 80) — NEW third top-level category, 3,390 rows pulled from `Frame & Hardware` + `Suspension` (both left in place, untouched) plus scattered stragglers. 3,069 rows held back. See HANDOFF_LOG.
...
**Accessories & Misc reclassification — drafted, dry-run tested, NOT yet run against live data** (session 81). Two discovery audits established this is NOT a "missing subcategory" problem like every prior category — it's rows genuinely misplaced at the **top-level category**. `fix_accessories_misc_taxonomy.mjs` reclassifies across categories (Wheels & Tires, Hardware Covers & General, Electrical, Handlebar & Controls, Foot Controls, Transmission & Clutch, Suspension), passed a 36-case regression suite, but the actual dry run against the live database has not been executed yet — Laken paused mid-session to get docs updated. **This is the first thing to run next session.**
...
| Item | Status |
|------|--------|
| display_category (**26 confirmed values** — Gaskets & Seals + Cables added session 77; Windshields & Fairings was already live but never recorded here; Tanks & Body added session 79; Frames & Suspension added session 80, a genuine net-new category; Dashes & Gauges and Footrests & Floorboards are in-place rebuilds of existing categories, not net-new; **Hardware, Covers & General added session 81**, a genuine net-new category; Wheels & Tires is an in-place rebuild, not net-new) | ✅ |
| display_subcategory across all **24** display categories | ✅ |
| **9 of 26 categories at ZERO null subcategories** (Cables, Carburetion & Fuel, Electrical, Engine, Gaskets & Seals, Handlebar & Controls, Lighting, Transmission & Clutch, Windshields & Fairings, Hardware Covers & General). **Brakes is NOT zero** — 54 rows deliberately held (session 79), not a gap. **Dashes & Gauges, Frames & Suspension, Foot Controls also NOT zero** (session 80) — 47 / 3,069 / 467 rows respectively deliberately held or awaiting a separate pass, not gaps. **Wheels & Tires also NOT zero** (session 81) — 5 rows deliberately held (standalone tools). **Full catalog health check (session 81) confirmed real counts for the first time**: 6,859 NULL rows catalog-wide (7.6%), concentrated in Accessories & Misc (3,203, 80.6% — worst, reclassification drafted but not yet applied), Riding Gear & Apparel (1,760, 41.9%), Fenders & Body (98, 79.0%), Tools & Chemicals (547, 29.9%), Foot Controls (454, 14.4%), Frame & Hardware (436, 18.3%), Suspension (99, 18.9%), Security & Covers (36, 17.0%), Seating (142, 3.8%) | ✅ |
...
| **Frames & Suspension — NEW top-level display_category** (session 80) — `fix_frames_suspension_taxonomy.mjs`. NEW third category (not a merge) — `Frame & Hardware` and `Suspension` both remain live, untouched, as their own categories; 3,390 rows pulled from them plus scattered stragglers, across 7 subcategories. Three rounds of exclusion-list fixes after discovering `Frame & Hardware`'s "Hardware & Fasteners" subcategory is a cross-system fastener bin (brake/engine/transmission bolt kits, not frame-specific) — bare HARDWARE/SPRING/FORK/SHOCK fallbacks were sweeping these in until excluded by other-system noun (BRAKE/ROTOR/ENGINE CASE/etc). Settled at 52.5% coverage by design; ~3,069 rows correctly held back as genuine cross-system fasteners | ✅ |
...
| **Accessories & Misc reclassification — drafted, dry-run tested, NOT yet run against live DB** (session 81) — `audit_accessories_misc_nulls.mjs` + `audit_accessories_misc_crossclassify.mjs` (discovery) → `fix_accessories_misc_taxonomy.mjs` (drafted). Confirmed this is a genuine cross-CATEGORY misplacement problem (not just missing subcategories, unlike every prior category) — 886/3,203 NULL rows (28%) already coverable by extending existing Hardware/Covers, Wheels & Tires, and Merchandising vocabulary. All 43 "Spool" rows in this bucket individually verified by name: 42 are spool-hub wheels/hubs/axles → Wheels & Tires, 1 (`Parkerized Spool Shifter Peg`, caught by Laken personally) → Foot Controls. New rules added for parts misplaced entirely at the category level (Starter Relay/Ignition Coil → Electrical, Grip Set → Handlebar & Controls, Shifter Peg → Foot Controls, Shifter Fork/Gear → Transmission & Clutch, Valve Stem Cover → Wheels & Tires, Damper Washer → Suspension). 36-case regression suite passed. **Actual dry run against live data not yet executed — first task for next session** | ⏳ Drafted + tested, not run |
