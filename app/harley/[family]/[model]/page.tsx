...
function CategoryTabBar({ categories, active, onChange }: {
  ...
  const tabs = [
    ...
    ...(categories ?? []).map(c => ({ name: c.name, label: CATEGORY_LABELS[c.name] ?? c.name, count: c.count })),
  ];
  ...
  function TabButton({ cat }: { cat: { name: string | null; label: string; count: number } }) {
    ...
    return (
      <motion.button
        ...
        style={{ flexShrink: 0, background: 'none', border: 'none', cursor: 'pointer', padding: 0, position: 'relative' }}
      >
        <div style={{
          padding: '7px 16px',
          position: 'relative', display: 'flex', alignItems: 'center', gap: 6,
          whiteSpace: 'nowrap', background: GOLD,
          borderTop: isActive ? '2px solid #f0d870' : '2px solid #8a6820',
          borderRight: '1px solid #8a6820', borderLeft: '1px solid #8a6820', borderBottom: 'none',
          boxShadow: isActive ? 'inset 0 -2px 8px rgba(0,0,0,0.3)' : 'inset 0 -3px 6px rgba(0,0,0,0.4)',
        }}>
          {isActive && <div style={{ position: 'absolute', bottom: -2, left: 0, right: 0, height: 3, background: GOLD, zIndex: 2 }} />}
          <span style={{
            fontFamily: 'var(--font-stencil, monospace)',
            fontSize: 10, letterSpacing: '0.12em',
            textTransform: 'uppercase', lineHeight: 1, color: BLACK,
            fontWeight: isActive ? 700 : 500, position: 'relative', zIndex: 1,
          }}>{cat.label}</span>
          {cat.count > 0 && (
            <span style={{ fontFamily: 'monospace', fontSize: 8, color: 'rgba(8,7,6,0.4)', position: 'relative', zIndex: 1 }}>{cat.count}</span>
          )}
        </div>
      </motion.button>
    );
  ...
  return (
    <div style={{
      background: BLACK, borderBottom: '2px solid #2a1e06',
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '6px 24px 0' }}>
        ...
        {row2.length > 0 && (
          <div style={{ display: 'flex', gap: 3, flexWrap: 'nowrap', overflowX: 'auto', scrollbarWidth: 'none' }}>
            {row2.map(cat => <TabButton key={cat.name ?? '__all2__'} cat={cat} />)}
          </div>
        )}
      </div>
    </div>
